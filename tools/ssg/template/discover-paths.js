  ...
  try {
    ...
    return Promise.all(
      files.flatMap(async (file) => {
        ...
        if (stat.isDirectory()) {
          return (await getRecursiveFileReads(filePath)).flatMap(
            (item) => item,
          );
        }
        return [filePath];
...
export async function discoverPaths() {
  const mainXmluiPath = path.resolve(import.meta.dirname, "src", "Main.xmlui");
  ...
  try {
    ...
    console.warn(
      `Warning: Could not read ${mainXmluiPath}, returning default path.`,
    );
    return ["/"];
  }
  ...
  if (PagesComp && PagesComp.children) {
    for (const page of PagesComp.children) {
      ...
      if (page.props.staticPaths) {
        try {
          const staticPaths = extractParam(
            {},
            page.props.staticPaths,
            undefined,
            true,
          );
  ...
  try {
    ...
    navPanelContent
      .filter(
        (filePath) =>
          filePath.endsWith(".md") && !filePath.startsWith("content/pages/"),
      )
