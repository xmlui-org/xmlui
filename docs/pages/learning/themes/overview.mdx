...
To remove the pain of setting all `width` and `height` occurrences in the previous example (when using layout properties), define a reusable component, `MyBox` with the wired dimension values; however, allow them to receive properties to set the border and color values:
