...
The XMLUI markup is almost identical to the HTML markup; however, there are a few differences. This section explains the markup fundamentals and treats the differences from HTML.
...
You can omit quotes when defining an attribute value, provided the value does not contain whitespaces and its characters are letters, numbers, and one of these characters: dash (`-`), underscore (`_`), dollar sign (`$`), dot (`.`), ...
