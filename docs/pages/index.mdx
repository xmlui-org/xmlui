...
We have a starter app you can use to get acquainted with XMLUI. Download its zip file from here (link placeholder) and unzip it into a working folder.

You need only a few things to try it:
- Your favorite code editor. (We suggest you use [Visual Studio Code](https://code.visualstudio.com/download). It is a free tool running on Windows, Mac, and Linux.)
...
When you click the "API-Aware" menu item, the app displays a list with data fetched from the SpaceX API.
...
This functionality is a cooperation of a couple of components within `App`: `NavLink`, `Page`, and `ApiAware`:

```ueml filename="Main.xmlui" {3,7,8}
<App>
  <NavPanel>
...
Components work together declaratively. `App` connects the `to` property of `NavLink` with the `url` property of `Page`. As a result, the particular page is displayed when you click the "API-Aware" menu item.

### Extracting Components

The page's content is outsourced into a reusable component, `ApiAware`, part of the application. Like other app-specific components, it is in the `components` folder.

```ueml filename="components/ApiAware.xmlui"
<Component name="ApiAware">
...
<Callout type="info" emoji="💡">
  To avoid code repetition or organize your code, you can extract markup snippets in **reusable component definitions**. When you change this component's definition, all occurrences will render the modified markup.
  Reusable components are stored in a separate `.xmlui` file within the `components` folder. The framework observes them and uses them in the markup.
</Callout>

This component displays a `List` that uses templates. Its `datasource` property specifies a URL to fetch the list's data.
...
```ueml copy {2-4} filename="components/ApiAware.xmlui" /datasource/
<List datasource="https://api.spacexdata.com/v3/history">
  <property name="itemTemplate">
    <Card title="{$item.title}" subTitle="{$item.details}" />
  </property>
...
<Callout type="info" emoji="💡">
  Template properties cannot be represented with a string; thus, they cannot be given with an attribute. You nest them into their related component with the **`<property>` tag**.
</Callout>
...
```ueml copy {2-3} filename="components/ApiAware.xmlui"
<Card title="{$item.title}" subTitle="{$item.details}">
...
```ueml copy filename="components/MyBox.xmlui" /onClick/ {5}
<Component name="MyBox">
  <Stack 
    padding="12"
    border="8px solid {$props.color}"
...
- Discover the source code files of the Starter App that were not mentioned in this article. We suggest you check the markup files in the `components` folder.
- Try the Personal Todo App tutorial. This article will teach you a few helpful components and simple patterns for creating apps.
...
- Observe the How-to topic to learn about patterns that help you in a particular scenario.