...
| 57 | [Option](./components/Option.mdx) | `Option` is a non-visual component describing a selection option. Other components (such as `Select`, `AutoComplete`, and others) may use nested `Option` instances from which the user can select. | stable |
...
| 68 | [Select](./components/Select.mdx) | Provides a dropdown with a list of options to choose from. | experimental |
| 69 | [Slider](./components/Slider.mdx) | The `Slider` component allows you to select a numeric value between a range specified by minimum and maximum values. | experimental |
