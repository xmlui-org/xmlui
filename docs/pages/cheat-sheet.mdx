...
import { COMPONENT_APP, THE_APP_COMPONENT, THE_APP_COMPONENT_LAYOUT_VARIANTS, XMLUI_EXPRESSIONS_EXPRESSION_SYNTAX } from '../meta/pages';
...
You can declare event handlers in the markup or the code-behind file. Event handlers can be XMLUI scripts or components that may act as event handlers. Currently, XMLUI has one component, `APICall`, with this trait.

**Variant 1: Markup with `on` attribute prefix**
...
**Variant 2: Markup with the `<event>` helper tag**

Alternatively, you can use the `<event>` helper tag with the `name` and `value` attributes:

```ueml copy
<Button label="Click me">
  <event name="click" value="toast('Clicked!')" />
</Button>
```

**Variant 3: Markup with the `<event>` helper tag and nested value**

Instead of using the `value` attribute of `<event>`, you can nest it into the element:
...
Declare a function in a code-behind file. You can assign the name of the function declared in the corresponding markup file to the event handler:
...
XMLUI supports auto-binding for event handlers in the code-behind file based on a particular function name convention. Your component must have an id, and the event handler function name must be `[componentId]_on[eventName]`, like in the following example:
...
The XMLUI engine supports components that can act as event handlers. To invoke an API endpoint from an event handler can be done by declaring an `APICall` component as an event handler. This option is available only with the component markup (not in the code-behind file) using the `<event>` helper tag:
...
XMLUI makes it easy to establish an application layout with the `App` component and other layout components like `Stack`, `FlowLayout`, and others.

The `App` component has a `layout` property that defines the initial layout of an app, including its header, navigation panel, main content, footer, and pages. For example, by setting the layout to `horizontal-sticky,` the navigation panel displays the items horizontally. As you scroll the content, the heading and the navigation panel dock to the top, while the footer sticks to the bottom.

```ueml copy
<App layout="horizontal-sticky">
```

You can learn more about the available app layouts <SmartLink href={THE_APP_COMPONENT_LAYOUT_VARIANTS}>here</SmartLink>.

These are the layout components that help you establish the app's layout:

|Component|Description|
|---|---|
|`App`|Provides the fundamental layout of the app. Learn more about using `App` <SmartLink href={THE_APP_COMPONENT}>here</SmartLink>, or check its <SmartLink href={COMPONENT_APP}>component reference</SmartLink>.|
