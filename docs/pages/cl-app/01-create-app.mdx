...
The application uses the `vertical-sticky` layout, which renders a vertical menu on the left side of the app and provides a sticky header and footer above and below the main content area. The `AppHeader` defines a title and allows a `ThemeChangerButton` to choose between light and dark themes.
