...
In your command prompt, select the downloaded sample's unzipped folder (with the cd command), and run `start.bat` on Windows or `start.sh` on Mac or Linux, which will start the web server and open your browser with the app. If you are using Mac or Linux, do not forget to add execution right to `start.sh` with this command line: `chmod +x start.sh`
