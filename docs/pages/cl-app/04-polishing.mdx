...
In the previous section, you created a `getSections` function within `Main.xmlui.xs`. Let's use the same function in the `Contact.xmlui.xs` code-behind file.

Create the `Contacts.xmlui.xs` code-behind file for `Contacts` (it in the `components` folder, beside `Contacts.xmlui`) and add the definition of `getSections` and a few other functions to it:
...
```ueml copy {2, 4-10} filename="Contacts.xmlui" /sectionBy="section"/
...
In this code snippet, the `sectionBy` property of `List` names the list's property for grouping items; it is the newly calculated `section` property of a contact record.

The highlighted `sectionTemplate` area is the markup that represents the header of a particular section (item group). The list passes the special `$item` property to the section template. Its value is an object with `sectionKey` holding the grouping value; `sectionItems` store the items within the particular group.
...
<Callout type="info" emoji="📔">
When you do not declare a section template, the items are still grouped, but the list does not indicate the section boundaries. Besides `sectionTemplate`, `List` provides a `sectionFooterTemplate`. With this property, you can declare a markup for the footer of a section.
</Callout>
...
```ueml copy {4-18} filename="Contacts.xmlui" /$space-normal/ /$space-tight/
<Component name="Contacts">
  <DataSource id="categories" url="/api/categories" />
  <DataSource id="contacts" url="/api/contacts/{$props.filter}" />
  <List 
...
With the help of the `defaultGroups` property of the `List`, you can define the ordering.
...
The source of this issue is the list's markup, which declares default sections. While those are helpful for the "All contacts" view, they result in unnecessary sections for filtered views.

Use a new property in `Contacts` (name it `showAllSections`) to check if the component should display each section:

```ueml copy {4-6} filename="Tasks.xmlui" /$props.showAllSections/
<!-- Omitted -->
<List 
  items="{contacts.value.map(t => ({...t, section: getSection(t)}))}"
...
When you pass any non-empty value (it can be anything), the highlighted markup sets the `defaultSection` property as if it were undefined and displays only the sections available by the data. Otherwise, it will use the listed sections in their definition order.

Add the `showAllSections` property to the "All contacts" view in `Main.xmlui`:

```ueml copy filename="Main.xmlui" /showAllSections="yes"/
<!-- Omitted -->
<Pages>
  <Page url="/">Dashboard</Page>
  <Page url="/contacts/all">
    <Contacts data="/api/contacts" showAllSections="yes" />
  </Page>
  <!-- Omitted -->
</Pages>
<!-- Omitted -->
