...
1. Navigate to the freshly created `ContactRow.xmlui` file and add a `DataSource` component before the `Card` with the `categories` identifier. This `DataSource` component will fetch all category data from the backend.

```ueml copy {2} filename="ContactRow.xmlui"
<Component name="ContactRow">
...
The `category` variable contains the category record associated with the item. The `findByField` function searches the first category item with the id the current contact row refers to (in its `categoryId` field). The variable has an `undefined` value if the item does not have a topic. 
...
<Callout type="info" emoji="📔">
You may have noticed that the definition of `ContactRow` includes a `DataSource` that fetches the contact categories:
...
The engine is smart enough to recognize that it should not fetch this endpoint every time a contact row is displayed. It will fetch the data only when the first visible row in the list is to be rendered and will use the cached set of categories for the subsequent rows.
</Callout>
