...
```ueml copy /5e9d0d95eda69955f709d1eb/ /$param/
<App 
  var.rocketData="">
...
<Playground
  ...
  api={{
    ...
    operations: {
      ...
      'post-fruit': {
        method: 'post',
        url: '/api/my-names/:id/:name',
        pathParamTypes: {
          id: "integer",
        },
        handler: `
          $state.names.push({id: $pathParams.id, name: $pathParams.name});
        `,
      },
    }
  }}
  app={`
    <App>
      ...
      <HStack>
        <APICall 
          id="addNameApi"
          url="/api/my-names/{$params[0]}/{$params[1]}" 
          method="post" />
