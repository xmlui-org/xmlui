...
<Callout type="warning" emoji="⚠️">
  ...
  The article contents below are obsolete.
</Callout>
...
You can bind the `data` property to a `DataSource` instance. Similarly to a single URL, `DataSource` fetches the data from that URL. However, when you need some additional configuration to issue a fetch request (for example, changing the `GET` method or adding some custom headers), `DataSource` provides that flexibility over a single string URL.
...
<Playground
  ...
  app={`
    <App>
      <DataSource id="rocketsData" url="https://api.spacexdata.com/v4/rockets" fetchDelay="{3000}" />
      <List data="{rocketsData}">
...
<DataSource id="spaceXData" url="https://api.spacexdata.com/v4/history" />
