...
For example, the following component uses a public API to query and show registered brewery information for Los Angeles:

```ueml copy /breweries.value.length/
<App>
  <DataSource id="breweries" url="https://api.openbrewerydb.org/breweries?by_city=Los%20Angeles" />
  <H2>Number of registered open breweries in Los Angeles: {breweries.value.length}</H2>
</App>  
```

This data query retrieves an array; thus, `breweries.value.length` represents the number of entries in the response.

<Playground
  previewOnly
  name="Example: Using value"
  app={`
    <App>
      <DataSource id="breweries" url="https://api.openbrewerydb.org/breweries?by_city=Los%20Angeles" />
      <H2>Number of registered open breweries in Los Angeles: {breweries.value.length}</H2>
...
```ueml copy /issData.refetch/
<App>
  <Button label="Ask for a new quote" onClick="quoteData.refetch()" />
  <DataSource id="quoteData" url="https://api.quotable.io/random" />
  <H2>{quoteData.value.content}</H2>
</App>  
```

This app uses a public API to obtain a random quote. Here, `quoteData.value` represents the data retrieved from the URL; the text displays the `content` field.

<Playground
  previewOnly
  name="Example: Using refetch()"
  app={`
    <App>
      <Button label="Ask for a new quote" onClick="quoteData.refetch()" />
      <DataSource id="quoteData" url="https://api.quotable.io/random" />
...
```ueml copy /when="{!issData.inProgress}"/ /when="{issData.inProgress}"/
<App>
  <Button label="Ask for a new quote" onClick="quoteData.refetch()" />
  <DataSource id="quoteData" url="https://api.quotable.io/random" />
  <Text when="{quoteData.inProgress}">Loading...</Text>
  <H2 when="{!quoteData.inProgress}">{quoteData.value.content}</H2>
</App>  
```

The app displays a "Loading..." message during the fetch. When the data is obtained, the app displays the ISS coordinates. Check how the markup leverages components' `when` attributes to hide and show them.

<Playground
  previewOnly
  name="Example: Testing whether data fetch is in progress"
  app={`
    <App>
      <Button label="Ask for a new quote" onClick="quoteData.refetch()" />
      <DataSource id="quoteData" url="https://api.quotable.io/random" />
...
### Polling a DataSource

Instead of manual polling, you can set a `DataSource` to use automatic, periodic polling. Use the `pollIntervalInSeconds` property as the following example shows:

```ueml copy
<App>
  <DataSource id="issData" url="http://api.open-notify.org/iss-now.json" 
    pollIntervalInSeconds="2"/>
  <H1>Polling ISS location every 2 seconds</H1>  
  <H2>Latitude: {issData.value.iss_position.latitude}</H2>
  <H2>Longitude: {issData.value.iss_position.longitude}</H2>
</App>  
```

This sample uses a public API to query the International Space Station's current coordinates. The code refreshes this data every two seconds.
...
Here, the `value` property of `H2` is set to the data retrieved by the `quoteData` data source.
