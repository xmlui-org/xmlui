...
You can use expressions in property and variable values. The engine recognizes them as wrapped between curly braces, such as `{count}`. They are not limited to just variables. Here, you can see `{count + 10}` being used in the label. 

```ueml copy {3}
<Button
  var.count="{0}"
  label="Click count: {count + 10}"
  onClick="count++" />
```
...
<VStack padding="1rem" gap="0.5rem" var.list="{[]}" var.toDisplay="{0}">
  ...
  <ChangeListener
    ...
    onDidChange="toDisplay = list.reduce((a, b) => a+b, 0) /* Summarize values in 'list' */" />
  <Text>{toDisplay}</Text>
</VStack>
```
