...
```ueml copy /when="{isDialogShown}"/ /onClose="isDialogShown = false"/ /onClick="isDialogShown = true"/
<App>
...
```ueml copy {2-8} /onClick="dialog.open()"/
<App>
  <ModalDialog id="dialog">
    <Form
      subject="{{ name: 'Leslie', age: 32 }}">
