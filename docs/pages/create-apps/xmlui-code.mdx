...
import { REUSABLE_COMPONENTS, SCRIPT_SYNTAX, SCRIPT_SYNTAX_EVENT_HANDLERS, XMLUI_EXPRESSIONS } from '../../meta/pages'
...
```ueml copy filename="Main.xmlui (markup)" /showDelay/
...
<Callout>
Component IDs are visible only in the scope of their markup (and associated code-behind) file.
</Callout>
...
```ueml copy /var.countTimes3="{3 * count}"/ /{countTimes3}/
...
<div style={{display: "flex", flexDirection: "row"}}>
  <div style={{width: "50%", paddingRight: "0.25rem"}}>
```ueml copy filename="Main.xmlui (markup)" /calculate/ /result/
<App>
  <Button 
    label="Calculate the sum of 1 to 100" 
    onClick="calculate()" />
...
<div style={{display: "flex", flexDirection: "row"}}>
  <div style={{width: "50%", paddingRight: "0.25rem"}}>
```ueml copy filename="Main.xmlui (markup)" /result/
<App>
  <Button 
    id="calcButton"
    label="Calculate the sum of 1 to 100" />
...
<Callout type="info" emoji="📔">
XMLUI understands your intention when defining an event handler. In addition to the arrow function syntax, it accepts several other alternatives, as here the <SmartLink href={SCRIPT_SYNTAX_EVENT_HANDLERS}>Event Handlers</SmartLink> section  describes.
</Callout>
...
These methods can use XMLUI Script statements like event handler functions. You can learn more about APIs in the <SmartLink href={REUSABLE_COMPONENTS}>Reusable Components</SmartLink> article.
...
```ueml /loopRuns = true; while (loopRuns) count++;/
