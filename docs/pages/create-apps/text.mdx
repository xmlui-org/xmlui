...
import { COMPONENT_HEADING, COMPONENT_MARKDOWN, COMPONENT_TEXT } from "../../meta/pages";
...
<Callout type="info" emoji="💡">
  Set the `verticalOverflow` property value to `hidden` to make the container crop the overflown part of a text.
</Callout>
...
<Callout type="info" emoji="💡">
  You can set `verticalOverflow` to `scroll` to make the container display a vertical scrollbar to reach the non-visible parts of the overflowing text.
</Callout>
...
<Callout type="info" emoji="💡">
  To enable and handle horizontal scrolling, use the `horizontalOverflow` property.
</Callout>
...
<Callout type="info" emoji="📔">
  When you set `horizontalOverflow` to `scroll,` it will automatically set `verticalOverflow` to `scroll` if the text exceeds its container vertically.
</Callout>
...
The `Heading` component renders text as headings.
It is useful for sectioning a body of text.
It has the same heading levels as the ones you can find in HTML set by the `level` property:
