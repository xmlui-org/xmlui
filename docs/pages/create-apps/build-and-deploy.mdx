...
| **`xmlui`** | The folder with XMLUI core framework, optional features, and third-party components |
| **`config.json`** | (*Optional*) The app configuration file |
| **`emulatedApi.js`** | (*Optional*) The API for the emulated backend |
| **`emulatedApiWorker.js`** | (*Optional*) The service worker for emulated backend |
...
Two files, `emulatedApi.js` and `emulatedApiWorker.js`, are present only in backend emulation apps.

If you have an emulated backend, the `emulatedApi.js` file should be loaded in `index.html` (before the XMLUI framework):
...
<head>
  ...
  <script src="./emulatedApi.js"></script>
...
The framework will automatically load the `emulatedApiWorker.js` file (if present and needed) while initializing.
