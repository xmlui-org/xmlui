...
The `multiCombobox` type represents a dropdown list with selectable items which can be filtered using the input field of the input control (similarly to the [Combobox](#combobox)).
...
<App>
  <Form
    subject="{{ size: 'xs' }}" 
    onSubmit="(toSave) => toast.success(JSON.stringify(toSave))">
    <FormItem type="multiCombobox" bindTo="size" label="Box size">
...
```ueml copy /patternInvalidSeverity/
<App>
...
<br/>
<Image alt="Submitting with Warnings" src="/resources/images/create-apps/using-forms-warning-dialog.png" />
