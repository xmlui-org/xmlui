...
Each theme variable is used to define the value of a property influencing the visual appearance of a particular part of the UI. The `<propertyName>` segment defines such a visual property. Here are a few examples:
...
For the complete list of available `<propertyName>` values, see the <SmartLink href={THEME_VARIABLES_PROPERTY_NAMES}>Theme Variables</SmartLink> article under "Component Reference".
