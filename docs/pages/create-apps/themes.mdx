...
```ueml copy /color-text-primary="orangered"/ /radius-Card="40px"/ /color-text-primary="mediumpurple"/
<App>
  <Theme color-text-primary="orangered" radius-Card="40px">
    <Card title="Card with custom text color and border radius" />
  </Theme>
  <Theme tone="dark" color-text-primary="mediumpurple">
...
<trait2>` segments describe one or two traits regarding a particular component.
    ...
    - `color-bg-Button-primary-solid`: Defines the background color of a button that is set up to use the `primary`theme
    color, displayed with the `solid` variant.
    - `color-border-Button-attention-ghost`: Specifies the border color of a ghost button displayed with the `attention`
    theme color.
...
<stateN>` values define visual states that may modify the appearance of a particular UI element or component (regardinga
    ...
    - `color-bg-Button-primary-solid--hover`: The background color to use with a `solid` button and with the `primary`
    theme color while the mouse hovers over it.
    - `color-border-Button-attention-ghost--focus`: Specifies the border color of a focused ghost button displayed with
...
<state>` values, see the <SmartLink href={THEME_VARIABLES_VISUAL_STATES}>Visual States</SmartLink> article in the "Theme
    ...
    <Playground fixedTheme={true} name="Input components" height={200} horizontal={true} app={inputs}/>
    ...
    with a theme with ten different `radius-
<component-name>` theme variables.
    ...
    The key to this behavior is the `Input` component name within the `radius-Input` and `color-bg-Input` variable
    names.
