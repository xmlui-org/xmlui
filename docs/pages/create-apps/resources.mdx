...
The `resources` folder is a dedicated folder that the XMLUI framework recognizes where all manner of resource files be placed.
Thus, there is no need to specifically define the path of resources, merely placing them in the folder is enough for the framework to load them in the application.
XMLUI is able to differentiate resource files based on their extensions and create handles from file names.
