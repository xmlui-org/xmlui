...
In the following code, `<propertyerty>` is a helper tag. XMLUI uses the `<propertyerty>` helper to specify the value of a property, which cannot be represented as a string.
...
<Callout type="info" emoji="💡">
  When properties cannot be represented with a string, you nest them into their related component with the **`<propertyerty>` tag**. Use the tag's **`name` attribute** to identify the property.
</Callout>
...
<Callout type="info" emoji="💡">
  If you prefer explicitly using the `<propertyerty>` tag, you can use it to represent string data; specify that with the `value` attribute.
</Callout>

```ueml /property/ /name/ /value/
<Button>
  <propertyerty name="label" value="Click me!" />
</Button>
```

<Callout type="info" emoji="💡">
  You can use the `<propertyerty>` tag with nested text as an alternative to the `value` attribute.
</Callout>
...
<Callout type="info" emoji="💡">
  You can use markup attributes to define event handlers for a particular event. Use an attribute with the `on` prefix for a particular event.
</Callout>
...
There are a few components (for example, `APICall`) the framework can resolve as the value of an event handler (these components declare the particular activity to execute). 

<Callout type="info" emoji="💡">
In such a case, you can use the `<event>` tag to set the event handler:
</Callout>

```ueml copy /onClick/
<Button label="Save">
  <event name="click">
    <ApiAction url="/api/customer" method="post" body="{customerData}">
  </event>
</Button>
```
...
And similarly to `<propertyerty>`, XMLUI accepts nested text for the variable's initial value.
