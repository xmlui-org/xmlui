...
A reusable component may define named `Slot` elements (each having a unique `name` property). The reusable component instance can provide separate content for these named slots.
...
<Playground
  ...
  components={[`
    <Component name="FancyCard">
      <VStack border="2px solid orangered" padding="$padding-loose">
        <var name="fancyTitle" value="*** {$props.title} ***" />
          <H1>{fancyTitle}</H1>
        <Slot name="titleTemplate">
          <H1>{fancyTitle}</H1>
        </Slot>
        <VStack backgroundColor="olive" padding="$padding-loose">
          <Slot />
        </VStack>
      </VStack>
    </Component>
  ...
  app={`
    <App>
      <FancyCard title="Demo Card">
        This is my fancy card content
      </FancyCard>
    </App>
  `}
/>
