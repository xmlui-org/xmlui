...
```ueml copy
<App>
  <Link to="https://docs.xmlui.com/">XMLUI introduction</Link>
  <Link to="https://docs.xmlui.com/" enabled="false">Disabled XMLUI introduction</Link>
</App>
```
...
<Callout type="info" emoji="💡">
  ...
  Note, that the `linkTo` property only works if the `title` property is set.
</Callout>
...
<App>
  <HStack>
    <Icon name="non-existing-icon" backgroundColor="olive" />
