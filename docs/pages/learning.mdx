...
```ueml copy
<MenuItem label="Delete completed tasks">
  <event name="click">
...
<H1>Expected delay="{calculatedDelay * (1 + reservePercentage * 0.01)} minutes"</H1>
```
...
<Button label="Calculate delay" onClick="
  ...
  const delay = calculatedDelay * (1 + reservePercentage * 0.01);
  console.log('Delay calculated:', delay);
" />
```

<Callout type="info" emoji="💡">
  You can move inline event handlers and use functions declared in **code-behind** files. Each markup file with a `.xmlui` extension may have a code-behind file with the same name as the markup file but using `.xmlui.xs`.
</Callout>
...
function handleClick() {
  console.log('Calculating delay...');
  const delay = calculatedDelay * 
    (1 + reservePercentage * 0.01);
  console.log('Delay calculated:', delay);
}
```
  </div>
...
<div style={{display: "flex", flexDirection: "row"}}>
  <div style={{width: "50%", paddingRight: "0.25rem"}}>
```ueml copy filename="Main.xmlui" /calcButton/
...
```
  ...
  <div style={{width: "50%", paddingLeft: "0.25rem"}}>
```js copy filename="Main.xmlui.xs" /calcButton_onClick/
