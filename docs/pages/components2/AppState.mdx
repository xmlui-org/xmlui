...
```ueml copy filename="Main.xmlui" /enhancedMode/
...
```ueml copy filename="Component1.xmlui" /enhancedMode/
...
```ueml copy filename="Component2.xmlui" /enhancedMode/
...
```ueml copy filename="Main.xmlui" /appState.update/ /appState.value/
<App>
...
```ueml copy filename="Component1.xmlui" /state.value.enhancedMode/
<Component name="Component1">
  <AppState id="state" />
...
```ueml copy filename="Component2.xmlui" /state.value.enhancedMode/
<Component name="Component2">
  <AppState id="state" />
...
This method updates the application state object bound to the `AppState` instance. The function's single argument is an object that specifies the new state value. If the argument is a hash object, it will be merged with the previous state value. Let's assume the previous state value was the following:
