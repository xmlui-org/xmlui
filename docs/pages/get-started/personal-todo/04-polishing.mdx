...
> **Note**: Here, the `items` property receives a list of items, each extended with the section information.
...
By specifying the default sections, the filtered lists also display them. So when you visit a filtered list, such as Overdue, it displays the empty sections:
