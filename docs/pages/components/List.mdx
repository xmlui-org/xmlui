...
This property adds a list of default groups for the `List` and displays the group headers in the specified order. If the data contains group headers not in this list, those headers are also displayed (after the ones in this list); however, their order is indeterministic.
...
This property set which attribute of the data is used to group the list items. If the attribute does not appear in the data items, it will be ignored.
...
<App>
  <List data='{${JSON.stringify(data, null, 2)}}' groupBy="category">
    ...
    <property name="groupFooterTemplate">
      <VStack verticalPadding="$space-normal">
        <ContentSeparator/>
      </VStack>
    </property>
  </List>
