...
Shows or hides the visibility of the close button on the dialog. The default value is `true`.
...
This method is used to close the `ModalDialog`. Invoke it using `modalId.close()` where `modalId` refers to a `ModalDialog` component. See the [`With Imperative API`](#with-imperative-api) subsection for an example.

> For the event of the same name, see [`close event`](#event).

### `open()` [#api]

This method is used to open the `ModalDialog`. Invoke it using `modalId.open()` where `modalId` refers to a `ModalDialog` component. See the [`With Imperative API`](#with-imperative-api) subsection for an example.

> For the event of the same name, see [`open event`](#event-1).
...
> For the API method of the same name, see [`close API method`](#api-1).
...
> For the API method of the same name, see [`open API method`](#api-2).
