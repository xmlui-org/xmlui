...
A `Splitter` component divides a container (such as a window, panel, pane, etc.) into two resizable sections.
These sections are identified by their name: the primary and secondary section.
The sections are divided by a draggable and stylable divider.

See also: [HSplitter](./HSplitter), [VSplitter](./VSplitter).

## Props

Most properties of the `Splitter` focus on the primary section, such as sizing.

### `floating`

Toggles whether the resizer is visible (`false`) or not (`true`) when not hovered or dragged.
The default value is `false`, so the resizer is visible.
...
(Fired when? at the start, during the whole time or when mouse button is released?)

```ueml copy
<Splitter height="100%" onResize="console.log('resized')">
  <Stack backgroundColor="lightblue" height="100%" />
  <Stack backgroundColor="darksalmon" height="100%" />
</Splitter>
```
...
The `Splitter` component provides these theme variables for appearance customization:
