...
is similar in nature to the [`Datasource`](
./Datasource.mdx
) component which retrieves data from the 
backend.

See examples on using the `ApiAction` component in [this tutorial](
../tutorials/working-with-apiactions.mdx
).
...
sending the request. Use the 
`rawBody`
 property to send another request body using its native 
