...
The `resultSelector` used here builds an array of the `properties` objects.
...
The `resultSelector` used here filters the array of the `properties` objects to include only contacts with non-null `custom_notes`.

```ueml copy
<DataSource 
  id="contacts" 
  resultSelector=`results.filter(contact => contact.properties.custom_notes !== null)
    .map(contact => contact.properties)`
  url="http:///{DOMAIN}/{CORS_PROXY}/api.hubapi.com/crm/v3/objects/contacts?properties=firstname,lastname,email,company,custom_notes"
  headers='{{"Authorization":"Bearer not-a-real-token"}}'
  /> 
````
