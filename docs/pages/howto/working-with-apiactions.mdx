import { COMPONENT_APIACTION, CREATE_THE_APP, DATA_FETCHING_WITH_DATASOURCE, GETTING_STARTED_WITH_XMLUI, USING_FORMS } from "../../meta/pages";
...
A companion tutorial is also available that deals with data loading titled <SmartLink href={DATA_FETCHING_WITH_DATASOURCE}>Data Fetching with DataSource</SmartLink>.
...
> **Note:** You are using the same emulated backend API as the one in the <SmartLink href={DATA_FETCHING_WITH_DATASOURCE}>`DataSource` tutorial</SmartLink>.
