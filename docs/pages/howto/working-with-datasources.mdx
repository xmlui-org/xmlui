...
A companion tutorial is also available that deals with data manipulation titled <SmartLink href={DATA_MANIPULATION_WITH_APIACTION}>Data Manipulation with ApiAction</SmartLink>.
...
XMLUI can quickly fetch data from an API. Most XMLUI components have a `datasource` property to handle API data loading conveniently. This property is useful for components that can display a list of items (such as the <SmartLink href={COMPONENT_LIST}>`List`</SmartLink> or <SmartLink href={COMPONENT_TABLE}>`Table`</SmartLink>).
