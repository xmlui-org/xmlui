...
None of the potential resolutions work. The reason is in the way `App` (as a layout container) works: it ignores its explicit height to handle all available <SmartLink href={COMPONENT_APP + "#layout"}>layout modes</SmartLink> (including the "sticky" layouts) consistently.
