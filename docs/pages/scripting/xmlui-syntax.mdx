...
XMLUI uses a scripting language, which is a subset of JavaScript. When an application runs, the framework precompiles scripts into an intermediate representation and processes (interprets) that. It uses the same expression evaluation and statement executing semantics as JavaScript.
...
XMLUI supports function declarations within event handlers and code behind files. When you declare functions, identifiers, destructuring constructs, and rest parameters can be used just like for <SmartLink href={XMLUI_EXPRESSIONS_ARROW_FUNCTIONS}>arrow expressions</SmartLink>.
...
functtions sumAll(...values) {
  return values.reduce((a, b) => a + b);
}
```

Just like in JavaScript, xmlui uses function hoisting: it does not matter where you declare a function; it will be moved to the top of its declaration scope. So, you can refer to a function (invoke it) before its declaration.

XMLUI support functions nested in other functions, so this declaration is valid:
