...
export function getLocalIconFolder(folderName: string) {
  const icons: Record<string, string> = import.meta.glob(`/src/icons/**/*.svg`, {
    import: 'default',
    eager: true,
    query: "?raw",
  });
  ...
  Object.entries(icons).forEach(([key, value]) => {
    ...
    if (folderName && !key.includes(`/src/icons/${folderName}/`)) {
      return;
    }
