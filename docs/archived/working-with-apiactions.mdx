import { COMPONENT_APIACTION, CREATE_THE_APP, DATA_FETCHING, GETTING_STARTED_WITH_XMLUI, USING_FORMS } from "../meta/pages";

# Data Manipulation with APICall

## Introduction

In this tutorial, you will learn how to manipulate data on the backend using the `APICall` component.
...
For a detailed rundown of the available props, events and API for the `APICall` component see the <SmartLink href={COMPONENT_APIACTION}>`APICall` reference docs</SmartLink>.

A companion tutorial is also available that deals with data loading titled <SmartLink href={DATA_FETCHING}>Data Fetching</SmartLink>.
...
<App layout="vertical-full-header">
  <AppHeader>
    <Text variant="title">APICall Tutorial</Text>
  </AppHeader>
  <Pages>
    <Page url="/">
...
> **Note:** You are using the same emulated backend API as the one in the <SmartLink href={DATA_FETCHING}>`DataSource` tutorial</SmartLink>.
...
```xmlui copy filename="Main.xmlui" {3-18}
<!-- Omitted -->
<Page url="/">
...
The button `click` event handler contains an `APICall` component that sends the predefined data in its `body` property to the backend.

It uses a `POST` HTTP method while calling the same API endpoint we used to list the groceries in the <SmartLink href="#data-display">Data Display section</SmartLink>.
...
In the real world, a form would be used to enter new data. Working with forms is a topic covered in the <SmartLink href={USING_FORMS}>Using Forms</SmartLink> article.

## Deleting Items

Adding a delete feature ensures unwanted shopping items can be removed. Change the item template of the `List` to display a Delete button to the right of each item with `APICall` set up to the URL that carries out the delete operation:
...
<!-- Omitted -->
```
This APICall (according to REST conventions) uses the `delete` method and includes the ID of the item to delete in its URL.
...
In most cases, you only want the users to delete items after confirmation. `APICall` allows you to display a confirmation dialog by setting a few properties. Add the highlighted lines to `APICall`:

```xmlui copy filename="Main.xmlui" {11-13}
<!-- Omitted -->
<Card>
  <HStack verticalAlignment="center" gap="0.5rem">
    <!-- Omitted -->
...
With this simple change, you need to confirm the deletion of an item; otherwise, `APICall` ignores invoking the requested backend API and, thus, does not delete the item:
...
It is also easy to use `APICall` to update data items. Update the markup according to the highlighted code to display a textbox with the item name and a button to save name changes:

```xmlui copy filename="Main.xmlui" {5, 7-15}
<!-- Omitted -->
<Card>
  <HStack verticalAlignment="center" gap="0.5rem">
    <VStack>
...
You can add query parameters to `APICall` using the `queryParams` property.
...
```xmlui copy filename="Main.xmlui" {15}
<!-- Omitted -->
<Page url="/">
  <Button label="Add New">
    <event name="click">
...
<!-- Omitted -->
```
When you click the Add New button, `APICall` will send a POST request with the `/api/shopping-list?highlight=true` URL, and the backend converts the name accordingly:
...
```xmlui copy {12-14} filename="Main.xmlui"
<APICall
  url="/api/shopping-list"
  method="post"
  body="{{
...
Several APIs require particular request headers to execute the operation. In such cases, you can use the `header` property of `APICall`.
...
```xmlui copy filename="Main.xmlui" {7}
<!-- Omitted -->
<Page url="/">
  <Button label="Add New">
    <event name="click">
...
The `APICall` component has several event handlers that the UI can utilize to perform additional activities, such as changing the UI.
...
```xmlui copy filename="Main.xmlui" {2, 7-9}
<!-- Omitted -->
...
Finally, add the event handlers to the `APICall` component and update the `url` prop to target a demonstration version (running slower) of the endpoint previously used:
  

```xmlui copy filename="Main.xmlui" {5, 8-10}
<!-- Omitted -->
<Button label="Change Name">
  <event name="click">
    <APICall
...
```xmlui copy filename="Main.xmlui" {13-14}
<!-- Omitted -->
<Card>
  <HStack verticalAlignment="center" gap="0.5rem">
    <VStack>
...
> **Note**: When `APICall` raises an error, it will display the error message.

> **Note**: You can use the data in the API response when displaying feedback.
> See <SmartLink href={COMPONENT_APIACTION + "#completednotificationmessage"}>`completedNotificationMessage`</SmartLink> 
> and <SmartLink href={COMPONENT_APIACTION + "#errornotificationmessage"}>`errorNotificationMessage`</SmartLink> in the `APICall` reference documentation for more information.