...
describe("discoverRoutes", () => {
  ...
  it("treats App.xmlui as an entrypoint synonym", async () => {
    const root = await makeProject({
      "src/App.xmlui": [
        '<Component name="InlineHeader">',
        '  <Text value="inline" />',
    ...
    try {
      ...
      expect(routes.allRoutes()).toContain("/");
      expect(routes.allRoutes()).toContain("/app-entry");
    } finally {
      process.chdir(cwd);
    }
  });

  it("keeps App.xmlui strict inside components folders", async () => {
    const root = await makeProject({
      "src/components/App.xmlui": [
        '<Component name="NotAnEntrypoint">',
        '  <Text value="inline" />',
        "</Component>",
        "<App>",
        "  <Pages>",
        '    <Page url="/should-not-discover" />',
        "  </Pages>",
        "</App>",
      ].join("\n"),
    });
    ...
    try {
      ...
      expect(routes.allRoutes()).toEqual(["/"]);
    } finally {
      process.chdir(cwd);
    }
  });
