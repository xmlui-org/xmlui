...
describe("Layout resolver", () => {
  ...
  alignmentCases.forEach((c) => {
    it(`horizontalAlignment: ${c}`, () => {
      const PROP = "horizontalAlignment";
      const result = resolveLayoutProps({ [PROP]: c });
      expect(result.nonCssProps[PROP]).toBe(c);
      expect(result.issues.has(PROP)).toBe(false);
    });
  });

  it("horizontalAlignment (themeVar)", () => {
    const PROP = "horizontalAlignment";
    const VALUE = THEME_ID;
    const result = resolveLayoutProps({ [PROP]: VALUE });
    expect(result.nonCssProps[PROP]).toBe(THEME_ID_VALUE);
    expect(result.issues.has(PROP)).toBe(false);
  });

  it("horizontalAlignment (issue)", () => {
    const PROP = "horizontalAlignment";
    const VALUE = "fake";
    const result = resolveLayoutProps({ [PROP]: VALUE });
    expect(result.nonCssProps[PROP]).toBe(VALUE);
    expect(result.issues.has(PROP)).toBe(true);
  });

  alignmentCases.forEach((c) => {
    it(`verticalAlignment: ${c}`, () => {
      const PROP = "verticalAlignment";
      const result = resolveLayoutProps({ [PROP]: c });
      expect(result.nonCssProps[PROP]).toBe(c);
      expect(result.issues.has(PROP)).toBe(false);
    });
  ...
  it("background: red", () => {
    ...
    const VALUE = "red";
    const result = resolveLayoutProps({ [PROP]: VALUE });
    expect(result.cssProps.backgroundColor).toBe(VALUE);
    expect(result.issues.has(PROP)).toBe(false);
  });
  ...
  it("background (themeVar)", () => {
    ...
    const VALUE = THEME_ID;
    const result = resolveLayoutProps({ [PROP]: VALUE });
    expect(result.cssProps.backgroundColor).toBe(THEME_ID_VALUE);
    expect(result.issues.has(PROP)).toBe(false);
  });
