...
export const CORPUS: CorpusCase[] = [
  ...
  { name: "arrow stored in an array, then called", kind: "binding", source: "[(x) => x * 2][0](3)", expected: 6 },
  { name: "arrow from a ternary branch", kind: "binding", source: "(flag ? (x) => x + 1 : (x) => x - 1)(10)", context: { flag: true }, expected: 11 },
  { name: "value-position arrow closing over scope", kind: "binding", source: "({ f: (x) => x * factor }).f(3)", context: { factor: 4 }, expected: 12 },
  { name: "arrow returning an arrow", kind: "binding", source: "({ f: (x) => (y) => x + y }).f(1)(2)", expected: 3 },
