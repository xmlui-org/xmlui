...
describe("Parser - regex literals", () => {
  ...
  regExpCases.forEach((regExp) => {
    it(`RegExp: ${regExp}`, () => {
      ...
      expect(result.value instanceof RegExp).equal(true);
    ...
    it(`RegExp in statement: ${regExp} #1`, () => {
      ...
      expect(stmt.declarations[0].expression!.type).equal("LitE");
      expect(stmt.declarations[0].expression!.value instanceof RegExp).equal(true);
