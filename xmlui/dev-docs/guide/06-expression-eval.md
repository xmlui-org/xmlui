...
**Timeout:** 1,000ms hard limit. If a sync evaluation takes longer (e.g., an accidentally infinite loop), it throws:
...
### 7. The `::` Operator (Global Scope Access)

XMLUI adds a custom operator `::` that bypasses all scopes and resolves directly from `globalThis`:

```javascript
::console.log("hello")    // accesses globalThis.console directly
::Math.PI                  // accesses globalThis.Math directly
::JSON.stringify(obj)      // accesses globalThis.JSON directly
```

Without `::`, identifiers are resolved through XMLUI's scope chain: block scopes → closures → parent threads → localContext → appContext → globalThis. The `::` operator skips straight to `globalThis`.
...
|---------|---------|---------------|
| Sync (bindings) | 1,000ms | No |
| Async (events) | `evalContext.timeout` | Yes |

The sync timeout prevents infinite loops in binding expressions from freezing the UI.

### 9. Missing JavaScript Features
...
| Optional catch binding (`catch { }`) | Not supported — must use `catch (e) { }` |
| `Symbol`, `WeakMap`, `WeakSet` | Not constructable (only 4 constructors allowed) |

### 10. Import System Restrictions
...
The `::` operator skips directly to step 6.
...
8. **1-second sync timeout.** Infinite loops in binding expressions are caught and killed. Event handlers have a configurable (or no) timeout.
