...
| `"build"` | Build-time validation diagnostic echoed at runtime by the analyzer pipeline. Payload: `code, severity, file, line, column, message`, optional `data`, `suggestions`. |
| `"errors"` | Structured error event from the exception pipeline. Payload: `code: ErrorDiagnosticCode`, `source`, `severity`, `message`, optional `componentUid`, `correlationId`. |
| `"audit"` | Audit-pipeline self-diagnostic emitted when the audit subsystem encounters a structural problem — redaction gap, sink failure, buffer overflow. Payload: `code: AuditDiagCode`, `severity`, `message`, optional `data`. |
| `"a11y"` | Accessibility linter finding emitted at runtime when `App.appGlobals.strictAccessibility` is truthy. Payload: `code: A11yCode`, `severity`, `componentName`, `message`, optional `fix`. |
