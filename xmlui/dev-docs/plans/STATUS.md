...
| 16 | Concurrent-state determinism | 🔄 | W4 | 2026-05 | W4 foundation started: `components-core/scheduler/` module shipped with diagnostics, FIFO/concurrent queue skeleton, happens-before render-phase helpers, replay stub, and `kind:"scheduler"` trace documentation; reducer emits `determinism-state-write-after-render` if a write occurs while the render-phase marker is active. Remaining: guide chapter, renderer marker integration, full action-invoker scheduler integration, floating-point/iteration audits, replay harness, and strict default flip. |
