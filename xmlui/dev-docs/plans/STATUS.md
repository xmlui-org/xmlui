...
| 10 | Defended routing | 🔄 | W4 | 2026-05 | W4 foundation started: `components-core/routing/` module shipped with diagnostics, constraint compiler/coercer, canonicaliser, and guard dispatcher; `<Page url>` constraints (`int`/`number`/`uuid`/`slug`/`bool`/`enum`/`length`/`regex`) compile to React Router paths and constrained `$routeParams` are exposed through routing state; non-matching constrained params route to `fallbackPath`; browser-driven `willNavigate` rejection is detected and reverted unless `appGlobals.guardOnPopState === false`; appGlobals for URL canonicalisation documented. Remaining: query constraints, full per-page `<Page guard>` execution, richer trace/error surfaces, docs, and strict default flip. |
