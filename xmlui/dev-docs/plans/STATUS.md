...
| 11 | i18n foundations | 🔄 | W4 | 2026-05 | W4 foundation started: `components-core/i18n/` module shipped with locale resolver, bundle store, and `Intl.*`-backed formatters; `App.locale`, `App.localeSource`, `App.availableLocales`, `App.setLocale()`, `App.translate()`/`App.t()`, `App.isRtlLocale()`, and format helpers are exposed on the app context; `<App locale>` and `<App direction>` props wired. Remaining: external bundle loading, `<I18n>` component, framework string extraction, ICU MessageFormat dependency/runtime, RTL SCSS audit, docs, and strict default flip. |
