...
| 06 | Cooperative concurrency | 🔄 | W3 | 2026-05 | W3-6 done (Phase 1 risk probe — public API only): `components-core/concurrency/` module shipped with `CancellationToken` (`aborted`/`reason`/`throwIfAborted`/`onAbort`/`signal`), `HandlerCancelledError`, `createCancellationToken()`, `HandlerPolicy` union (`parallel`/`single-flight`/`queue`/`drop-while-running`), `HandlerCoordinator` interface + pass-through `createHandlerCoordinator()` stub, and `ConcurrencyCode` (all `concurrency-*` prefixed). `kind:"concurrency"` registered on `XsLogEntry`. `appGlobals.strictConcurrency` (default `false`) + `appGlobals.defaultHandlerTimeoutMs` (default `30000`) documented. `event-handlers.ts` injects a fresh `$cancel` token into every async handler's `localContext`; aborted in `finally` with `"user"`/`"unmount"` reason. `$cancel` registered as a framework variable. Default behaviour unchanged — coordinator runtime (supersede / queue / drop / timeout enforcement) lands in W7-1. |
...
| 11 | i18n foundations | 🔄 | W4 | 2026-05 | W4 i18n completion pass landed: bundle store is seeded with framework English strings, `<App localeBundles>` and `App.registerLocaleBundle(s)` load inline/URL bundles, `App.translate()` uses ICU-style variable/plural/select/number/date/time formatting with diagnostics, `<I18n>` renders bundle keys with variable props and named slot placeholders, and Form button defaults now resolve through translation keys. Remaining: broad built-in string extraction beyond Form, RTL SCSS audit/linting, docs, and strict default flip. |
