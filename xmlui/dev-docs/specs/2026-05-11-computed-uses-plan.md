...
**Виправлення:** Константа `JS_BUILTIN_GLOBALS` містить стандартні JavaScript globals (`JSON`, `Math`, `Date`, `Array`, тощо) і вони фільтруються з `totalFree`:
```ts
for (const d of usedHere)
  if (!localDeclared.has(d) && !JS_BUILTIN_GLOBALS.has(d)) totalFree.add(d);
```

**Слід мати на увазі:** `collectVariableDependencies` збирає **всі** ідентифікатори у виразі без фільтрації built-ins. Якщо у XMLUI-виразі використовується `JSON.stringify(x)`, `Math.max(a, b)`, `Date.now()` тощо — ці глобальні об'єкти з'являться у залежностях. Оскільки вони ніколи не знаходяться в app state, вони мають бути відфільтровані **до** обчислення `totalFree`, інакше будь-який контейнер що використовує такі вирази отримає помилковий `computedUses` і буде ізольований від батьківського стану.
...
**Слід мати на увазі:** `JS_BUILTIN_GLOBALS` (константа в `computedUses.ts`) містить всі стандартні JS globals і вони фільтруються **до** обчислення `totalFree`. При додаванні нових виразів у XMLUI що використовують нестандартні globals (наприклад, polyfills або third-party) — ці globals теж треба додати до `JS_BUILTIN_GLOBALS`, інакше вони стануть помилковими залежностями.
