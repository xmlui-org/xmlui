...
### 7. Двошарова ізоляція (defense in depth)
...
| `xmlui/src/components-core/prepare/computedUses.ts` | Create + Fix | `computeUsesForTree`, `computeUsesForSubtree`, `IMPLICIT_CONTAINER_COMPONENT_NAMES`; встановлює `computedUses` тільки коли `totalFree.size > 0` |
| `xmlui/tests/prepare/computedUses.test.ts` | Create | unit-тести алгоритму: базові випадки, UID-escaping через uses-контейнери, 3-рівневі non-container ланцюги, власний uid контейнера що escapes до батька |
