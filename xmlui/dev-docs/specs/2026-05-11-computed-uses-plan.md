...
**Виправлення:** Замість хардкодованого списку — перевірка `name in globalThis`, яка точно відображає поведінку evaluator (він теж падає на `globalThis` як останній резолвер):
```ts
const isGlobalThisIdentifier = (name: string): boolean => name in globalThis;

for (const d of usedHere)
  if (!localDeclared.has(d) && !isGlobalThisIdentifier(d)) totalFree.add(d);
```

**Слід мати на увазі:** `collectVariableDependencies` збирає **всі** ідентифікатори у виразі без фільтрації built-ins. Якщо у XMLUI-виразі використовується `JSON.stringify(x)`, `Math.max(a, b)`, `Date.now()` тощо — ці глобальні об'єкти з'являться у залежностях. Підхід через `globalThis` автоматично охоплює нові ECMAScript globals (наприклад, `Temporal`), platform polyfills і environment-специфічні globals (browser: `window`/`document`; Node.js: `process`) — без ручного обслуговування.

Важлива умова: XMLUI-змінні не повинні називатись так само як JS globals (наприклад, `var.JSON`, `var.window`). Якщо такий конфлікт стається — ідентифікатор буде неправильно відфільтрований як builtin.
...
**Слід мати на увазі:** фільтрація відбувається через `name in globalThis`, що точно дзеркалює логіку evaluator (він теж падає на `globalThis` в останню чергу). Перевірка автоматична — нові JS globals (наприклад, `Temporal`) і polyfills покриваються без змін коду. Немає хардкодованого списку що треба підтримувати вручну.
