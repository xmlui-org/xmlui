...
### 12. `scriptCollected` компоненти виключені зі звуження `computedUses`

Компоненти зі `scriptCollected` (user-defined `.xmlui` компоненти з `<script>` тегами або `.xs` файлами) **не отримують** `computedUses`. Причина: алгоритм аналізує лише шаблонні вирази, але `.xs` функції можуть звертатися до **будь-яких** глобальних змінних стану — включаючи ті, що взагалі не згадуються в шаблоні.

**Слід мати на увазі:** вбудовані XMLUI компоненти (`Select`, `Table`, `List`, `DataGrid`) не мають `scriptCollected` — вони реалізовані у TypeScript, не в `.xs`. Тому `computedUses` для них встановлюється коректно. Компоненти лише з `vars` (наприклад, `VStack var.x="{...}"`) теж не мають `scriptCollected` — звуження для них безпечне.

Звуження для `scriptCollected` компонентів потребує AST-аналізу тіл функцій та транзитивного відстеження залежностей через `import` ланцюжки (наприклад, `import { fn } from 'shared.xs'` де `fn` звертається до `serverConfig`). Це нетривіальна задача, що відкладена на майбутнє (див. TODO).
...
### Оптимізація `computedUses` для компонентів зі `scriptCollected`

Наразі компоненти зі `scriptCollected` (user-defined `.xmlui` з `<script>` або `.xs` файлами) повністю виключені зі звуження — вони отримують повний батьківський стан. Це коректно, але залишає потенційну оптимізацію на столі.

**Що потрібно для реалізації:**
1. AST-аналіз тіл функцій у `scriptCollected` — збирати ідентифікатори що читаються/пишуться у функціях (не лише у шаблонних виразах).
