...
export function splicePreservingInteractions(logs: any[], maxSize: number): void {
  const preserved = new Set(["interaction", "navigate", "api:start", "api:complete", "api:error", "handler:start", "handler:complete", "modal:show", "modal:confirm", "modal:cancel", "toast", "submenu:open"]);
  const keep: any[] = [];
  const evictable: any[] = [];
