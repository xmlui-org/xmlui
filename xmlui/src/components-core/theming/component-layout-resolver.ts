...
import type { LayoutContext, StylePropsResolver } from "../../abstractions/RendererDefs";
...
const cssPropertySet = new Set(allCssProperties);
function camelToKebab(key) {
  // Handle 'ms' prefix specifically, as it's lowercase
  if (key.startsWith('ms')) {
    key = '-' + key;
  }

  // This regex finds all uppercase letters and prepends a '-'
  // then converts the whole string to lowercase.
  return key.replace(/[A-Z]/g, match => `-${match.toLowerCase()}`);
}

export function isStyleProps(key) {
  // 1. Always allow CSS Custom Properties
  if (key.startsWith('--')) {
    return true;
  }

  // 2. Convert the React-style camelCase key to CSS-style kebab-case
  const kebabKey = camelToKebab(key);
  if(key === "orientation"){
    debugger;
  }

  // 3. Check if the kebab-case key exists in our Set
  return cssPropertySet.has(kebabKey);
}
...
export function resolveComponentLayoutProps(
  ...
  for (const [key, value] of Object.entries(layoutProps)) {
    const parsed = parseLayoutProperty(key, false, value);
...
export function extendLayoutPropsWithExtra(result: ResolvedComponentLayout, resolvedProps: Record<string, any>, stylePropsResolver: StylePropsResolver) {
  console.log(result);
  return result;
}

function cleanUpStyleObject(result: CssPropsWithStates) {
  if(!result){
    return;
  }
  Object.keys(result).forEach(propName => {
    if (propName === "states") {
      Object.keys(result[propName]).forEach(statePropsName => {
        cleanUpStyleObject(result.states[statePropsName]);
      })
      return;
    }
    if (!isStyleProps(propName)) {
      delete result[propName];
    }
  });
}

export function cleanUpLayoutProps(result: ResolvedComponentLayout){
  cleanUpStyleObject(result[BASE_COMPONENT_PART]?.baseStyles);
  Object.keys(result[BASE_COMPONENT_PART]?.responsiveStyles || {}).forEach(propName => {
    cleanUpStyleObject(result[BASE_COMPONENT_PART]?.responsiveStyles[propName]);
  })
  return result;
}
