 ...
 */
export function StyleProvider({ children, styleRegistry = new StyleRegistry() }: StyleProviderProps) {
...
export function useStyles(styles: StyleObjectType): string {
  ...
  const { className, styleHash } = registry.register(styles);
  ...
  useEffect(() => {
    ...
    return () => {
      const newCount = registry.decrementRef(styleHash);
      ...
      setTimeout(() => {
        ...
        if (registry.getRefCount(styleHash) === 0 && !registry.ssrHashes.has(styleHash)) {
          ...
          document
            .querySelector(`style[data-style-hash="${styleHash}"]`)
            ?.remove();
        }
