...
export const tooltipComponentRenderer = wrapComponent("Tooltip", TooltipMd, {
  exclude: ["tooltipTemplate", "text"],
  customRender(props, { node, extractValue, renderChild, layoutContext }) {
    ...
    return (
      <Tooltip
        {...props}
        text={extractValue.asOptionalString(node.props?.text) ?? ""}
        tooltipTemplate={renderChild(node.props?.tooltipTemplate)}
      >
