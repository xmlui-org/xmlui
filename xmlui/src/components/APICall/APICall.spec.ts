            ...
            "malformed-json": {
              ...
              handler: `
                if (typeof $requestBody === "string") {
                  throw Errors.HttpError(400, { message: "Malformed JSON body" });
                }
                return { success: true };
              `,
            },
...
test.describe("State Tracking", () => {
  ...
  test("error object contains statusCode, message, details, and response when handled by onError", async ({
    initTestBed,
    createButtonDriver,
  }) => {
    ...
    const { testStateDriver } = await initTestBed(`
      <Fragment>
        <APICall
          ...
          onMockExecute="() => BigInt('simulated error')"
        />
        <Button testId="trigger" onClick="api.execute()" label="execute"/>
      </Fragment>
    `);
  ...
  test("mock data list example: adds item via mockExecute", async ({
    ...
    const { testStateDriver } = await initTestBed(`
      <App var.mockData="{[
        ...
        <List testId="list" data="{myData.value}">
          <H3>{$item.name}</H3>
        </List>
        <Button testId="addBtn">
          <event name="click">
            <APICall url="/api/users" method="post" body="{{ name: 'New User' }}"
              ...
              onMockExecute="() => (mockData = [...mockData, { id: mockData.length + 1, name: 'New User' }])" />
          </event>
          Add user
        </Button>
      </App>
