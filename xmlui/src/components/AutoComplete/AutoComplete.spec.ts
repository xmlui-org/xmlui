...
test("bindTo syncs $data and value", async ({ initTestBed, page }) => {
  await initTestBed(`
    <Form hideButtonRow="true">
      <AutoComplete id="boundAutoComplete" bindTo="selection">
        <Option value="1" label="One" />
        <Option value="2" label="Two" />
      </AutoComplete>
      <Button testId="setBtn" onClick="boundAutoComplete.setValue('2')" />
      <Text testId="dataValue">{$data.selection}</Text>
      <Text testId="compValue">{boundAutoComplete.value}</Text>
    </Form>
  `);
...
test.describe("Validation Feedback", () => {
  ...
  test("prop on component overrides form default", async ({ initTestBed, page }) => {
    await initTestBed(`
      <Form verboseValidationFeedback="{true}">
        <AutoComplete testId="input" bindTo="input" verboseValidationFeedback="{false}" required="{true}">
          <Option value="1" label="Option 1" />
        </AutoComplete>
        <Button testId="submit" type="submit">Submit</Button>
      </Form>
