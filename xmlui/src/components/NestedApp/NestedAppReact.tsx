...
export const NestedApp = memo(function NestedApp({
  ...
  useEffect(() => {
    ...
    const compoundComponents: CompoundComponentDef[] = resolvedComponents ?? (components ?? []).map((src) => {
  ...
  }, [onInit, activeTheme, app, resolvedApp, componentRegistry, components, config?.appGlobals, config?.defaultTheme, config?.name, config?.resources, config?.themes, height, mock, parentInterceptorContext, style, theme.themeStyles, toneToApply, refreshVersion, resolvedComponents]);
