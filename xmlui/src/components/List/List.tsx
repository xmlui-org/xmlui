...
export const listRenderer = wrapRuntimeComponent({
  name: COMP,
  metadata: ListMd,
  renderer: ({ adapter }) => {
    const itemTemplate =
      templateChildren(adapter.node, "itemTemplate") ?? nonPropertyChildren(adapter.node.children);
    const emptyListTemplate = templateChildren(adapter.node, "emptyListTemplate");
    ...
    const hideEmptyGroups = adapter.booleanProp("hideEmptyGroups", defaultProps.hideEmptyGroups);
    const syncAdapter = useMemo(() => {
      if (!syncWithVar || !VALID_IDENTIFIER_RE.test(syncWithVar) || !hasSyncTarget(adapter.scope, syncWithVar)) {
        return undefined;
      }
      return {
        value: readLocal(adapter.scope, syncWithVar),
        update: ({ selectedIds }: { selectedIds: any[] }) => {
    ...
    }, [adapter.scope, syncWithVar]);

    const renderWithContext = (
      template: ComponentDef | Array<ComponentDef> | undefined,
      ...
      return adapter.context.renderChildren((Array.isArray(template) ? template : [template]) as any, itemScope);
    };
    ...
    const listContent = (
      <ListNative
        ...
        orderBy={adapter.prop("orderBy") as any}
        availableGroups={arrayValue(adapter.prop("availableGroups")).map(String)}
        scrollAnchor={adapter.stringProp("scrollAnchor", defaultProps.scrollAnchor) as any}
        pageInfo={adapter.prop("pageInfo") as any}
        idKey={idKey}
        onContextMenu={adapter.node.events.contextMenu ? (() => void adapter.event("contextMenu")()) : undefined}
        requestFetchPrevPage={adapter.node.events.requestFetchPrevPage ? (() => void adapter.event("requestFetchPrevPage")()) : undefined}
        requestFetchNextPage={adapter.node.events.requestFetchNextPage ? (() => void adapter.event("requestFetchNextPage")()) : undefined}
        emptyListPlaceholder={
          emptyListTemplate
            ? renderWithContext(emptyListTemplate as ComponentDef[], {})
            : undefined
        }
        ...
        initiallySelected={arrayValue(adapter.prop("initiallySelected")) as any}
        syncWithAppState={syncAdapter}
        rowUnselectablePredicate={functionValue(adapter.prop("rowUnselectablePredicate")) as any}
        hideSelectionCheckboxes={adapter.booleanProp("hideSelectionCheckboxes", defaultProps.hideSelectionCheckboxes)}
        selectionCheckboxPosition={adapter.stringProp("selectionCheckboxPosition", defaultProps.selectionCheckboxPosition) as any}
        selectionCheckboxAnchor={adapter.stringProp("selectionCheckboxAnchor", defaultProps.selectionCheckboxAnchor) as any}
        selectionCheckboxOffsetX={sizeProp(adapter.prop("selectionCheckboxOffsetX"), defaultProps.selectionCheckboxOffsetX)}
        selectionCheckboxOffsetY={sizeProp(adapter.prop("selectionCheckboxOffsetY"), defaultProps.selectionCheckboxOffsetY)}
        ...
        onSelectAllAction={(row, selectedItems, selectedIds) => void adapter.event("selectAllAction")(row, selectedItems, selectedIds)}
        onCutAction={(row, selectedItems, selectedIds) => void adapter.event("cutAction")(row, selectedItems, selectedIds)}
        onCopyAction={(row, selectedItems, selectedIds) => void adapter.event("copyAction")(row, selectedItems, selectedIds)}
        onPasteAction={(row, selectedItems, selectedIds) => void adapter.event("pasteAction")(row, selectedItems, selectedIds)}
        onDeleteAction={(row, selectedItems, selectedIds) => void adapter.event("deleteAction")(row, selectedItems, selectedIds)}
        rowDoubleClick={(item) => void adapter.event("rowDoubleClick")(item)}
        onScroll={(event) => void adapter.event("scroll")(event)}
        keyBindings={recordValue(adapter.prop("keyBindings"))}
        itemRenderer={hasItemTemplate ? ((item, _key, rowIndex, count, isSelected) =>
          renderWithContext(itemTemplate as ComponentDef[], {
            $item: item,
            $itemIndex: rowIndex,
            $isFirst: rowIndex === 0,
            $isLast: rowIndex === count - 1,
        ...
        sectionRenderer={groupHeaderTemplate ? ((group, _key) =>
          shouldRenderGroupTemplate(group, hideEmptyGroups)
            ? renderWithContext(groupHeaderTemplate as ComponentDef[], groupContext(group))
            : null) : undefined}
        sectionFooterRenderer={groupFooterTemplate ? ((group, _key) =>
          shouldRenderGroupTemplate(group, hideEmptyGroups)
            ? renderWithContext(groupFooterTemplate as ComponentDef[], groupContext(group))
            : null) : undefined}
      />
    );
...
function sizeProp(value: unknown, fallback: string): string {
  ...
  return cssSize(fallback);
}

function cssSize(value: string): string {
  const trimmed = value.trim();
  if (trimmed.startsWith("$")) {
    return `var(--xmlui-${trimmed.slice(1)})`;
  }
  return trimmed || "8px";
}

function groupContext(group: any): Record<string, unknown> {
  const groupInfo = {
    id: group?.id,
...
function shouldRenderGroupTemplate(group: any, hideEmptyGroups: boolean): boolean {
  return (group?.items?.length ?? 0) > 0 || !hideEmptyGroups;
}
...
function shouldWriteSyncSelection(currentValue: unknown, selectedIds: any[]): boolean {
