...
For the `defaultSections` property to work, the data must be sectioned using the [`groupBy`](#groupBy) property,
...
<Callout type="info" emoji="📔">
For the `groupBy` property to work, either a [`sectionTemplate`](#sectiontemplate)
