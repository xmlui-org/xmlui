...
test.describe("DatePickerNew - smoke", () => {
  ...
  test("inline renders only the calendar, no input control", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew inline testId="dp" dateFormat="MM/dd/yyyy" initialValue="05/25/2024" />`);
  ...
  test("inline calendar is interactive (days are clickable)", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew inline testId="dp" mode="single" dateFormat="MM/dd/yyyy" initialValue="05/15/2024" />`);
    const root = page.getByTestId("dp");
    await expect(root.locator('[data-part="table"]').first()).toBeVisible();
...
// the core DatePicker.
test.describe("DatePickerNew - disabled", () => {
  test("the control shows a not-allowed cursor", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" enabled="false" initialValue="05/25/2024" />`);
...
test.describe("DatePickerNew - readOnly", () => {
  test("the input cannot be typed into", async ({ page, initTestBed }) => {
    await initTestBed(
      `<DatePickerNew testId="dp" readOnly="true" dateFormat="MM/dd/yyyy" initialValue="05/26/2024" />`,
    );
...
test.describe("DatePickerNew - month & year navigation", () => {
  ...
  test("the month picker moves the visible month, even after chevron navigation", async ({
    ...
    await initTestBed(`<DatePickerNew testId="dp" dateFormat="MM/dd/yyyy" initialValue="05/15/2024" />`);
    await openCalendar(page);
  ...
  test("range: selecting a start in the second visible month does not shift the view", async ({
    ...
    await initTestBed(
      `<DatePickerNew testId="dp" mode="range" dateFormat="MM/dd/yyyy" initialValue="{{ from: '05/10/2024', to: '05/20/2024' }}" />`,
    );
    await openCalendar(page);
...
test.describe("DatePickerNew - trigger vs typing", () => {
  test("clicking the editable input focuses it and does not open the calendar", async ({
    page,
    initTestBed,
  }) => {
    await initTestBed(`<DatePickerNew testId="dp" dateFormat="MM/dd/yyyy" initialValue="05/25/2024" />`);
    const input = page.getByTestId("dp").locator("input").first();
    await input.click();
  ...
  test("a read-only input shows a pointer cursor (it is a trigger, not editable)", async ({
    ...
    await initTestBed(`<DatePickerNew testId="dp" readOnly="true" initialValue="05/25/2024" />`);
    const input = page.getByTestId("dp").locator("input").first();
  ...
  test("full-width keeps the number input narrow with clickable chrome", async ({
    ...
    await initTestBed(`<DatePickerNew testId="dp" width="100%" dateFormat="MM/dd/yyyy" initialValue="05/25/2024" />`);
  ...
  test("full-width: start adornment hugs the left edge, end adornment the right", async ({
    ...
    await initTestBed(`<DatePickerNew testId="dp" width="100%" startText="From" endText="UTC" dateFormat="MM/dd/yyyy" initialValue="05/25/2024" />`);
    const control = page.getByTestId("dp").locator('[data-part="control"]');
...
test.describe("DatePickerNew - mobile infinite scroll", () => {
...
test.describe("Basic Functionality", () => {
  test("applies a non-default dateFormat to the input", async ({ page, initTestBed }) => {
    await initTestBed(
      `<DatePickerNew testId="dp" dateFormat="yyyy-MM-dd" initialValue="2024-05-25" />`,
    );
  ...
  test("shows the date-format mask as the input placeholder when empty", async ({
    ...
    await initTestBed(`<DatePickerNew testId="dp" dateFormat="MM/dd/yyyy" />`);
  ...
  test("shows week numbers when showWeekNumber is enabled", async ({ page, initTestBed }) => {
    await initTestBed(
      `<DatePickerNew testId="dp" showWeekNumber="true" dateFormat="MM/dd/yyyy" initialValue="05/15/2024" />`,
    );
    await openCalendar(page);
  ...
  test("renders start and end adornments together", async ({ page, initTestBed }) => {
    await initTestBed(
      `<DatePickerNew testId="dp" startText="From" endText="Select" startIcon="date" endIcon="date" initialValue="05/25/2024" />`,
    );
    const root = page.getByTestId("dp");
...
test.describe("Accessibility", () => {
  test("respects the autoFocus property", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" autoFocus="true" />`);
...
test.describe("Event Handling", () => {
  test("fires gotFocus when focused", async ({ page, initTestBed }) => {
    const { testStateDriver } = await initTestBed(
      `<DatePickerNew testId="dp" onGotFocus="testState = 'focused'" />`,
    );
  ...
  test("fires lostFocus when blurred", async ({ page, initTestBed }) => {
    const { testStateDriver } = await initTestBed(
      `<DatePickerNew testId="dp" onLostFocus="testState = 'blurred'" />`,
    );
    const input = page.getByTestId("dp").locator("input").first();
    await input.focus();
    await expect(input).toBeFocused();
...
test.describe("Theme Variables", () => {
  test("applies the error validation border color", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" validationStatus="error" />`, {
      testThemeVars: { "borderColor-DatePickerNew--error": "rgb(220, 38, 38)" },
    });
    await expect(
  ...
  test("applies the warning validation border color", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" validationStatus="warning" />`, {
      testThemeVars: { "borderColor-DatePickerNew--warning": "rgb(255, 165, 0)" },
    });
    await expect(
      page.getByTestId("dp").locator('[data-part="control"]').first(),
  ...
  test("applies the valid validation border color", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" validationStatus="valid" />`, {
      testThemeVars: { "borderColor-DatePickerNew--success": "rgb(0, 128, 0)" },
    });
    await expect(
      page.getByTestId("dp").locator('[data-part="control"]').first(),
  ...
  test("weekday header uses the configured textColor-weekday", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" inline />`, {
      testThemeVars: { "textColor-weekday-DatePickerNew": "rgb(123, 45, 67)" },
    });
  ...
  test("today cell paints a ring, not a background fill", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" inline />`, {
      testThemeVars: {
        "backgroundColor-day-DatePickerNew--today": "transparent",
        "borderColor-selectedItem-DatePickerNew": "rgb(11, 22, 33)",
        "borderWidth-day-DatePickerNew--today": "2px",
      },
    });
    ...
    await initTestBed(
      `<DatePickerNew testId="dp" mode="single" dateFormat="MM/dd/yyyy" initialValue="05/15/2024" />`,
      {
        testThemeVars: {
          ...
          "textColor-day-DatePickerNew--selected": "rgb(255, 255, 255)",
        },
      },
    );
    await openCalendar(page);
  ...
  test("range middle uses the configured rangeMiddle colors", async ({ page, initTestBed }) => {
    await initTestBed(
      ...
      {
        testThemeVars: {
          "backgroundColor-day-DatePickerNew--rangeMiddle": "rgb(230, 240, 255)",
          "textColor-day-DatePickerNew--rangeMiddle": "rgb(11, 22, 33)",
        },
      },
    );
    await openCalendar(page);
...
test.describe("Disabled Days", () => {
  test("startDate blocks selecting an earlier day", async ({ page, initTestBed }) => {
    await initTestBed(`
      <Fragment>
        <DatePickerNew id="dp" testId="dp" mode="single" dateFormat="MM/dd/yyyy"
  ...
  test("disabled day paints the configured disabled colors", async ({ page, initTestBed }) => {
    await initTestBed(
      `<DatePickerNew testId="dp" inline dateFormat="MM/dd/yyyy" initialValue="05/15/2024" disabledDates="{['05/20/2024']}" />`,
      {
        testThemeVars: {
          "backgroundColor-day-DatePickerNew--disabled": "rgb(220, 38, 38)",
          "textColor-day-DatePickerNew--disabled": "rgb(255, 255, 255)",
        },
      },
    );
  ...
  test("clicking a disabled date does not change the value", async ({ page, initTestBed }) => {
    await initTestBed(`
      <Fragment>
        <DatePickerNew id="dp" testId="dp" inline dateFormat="MM/dd/yyyy"
          initialValue="05/15/2024" disabledDates="{['05/20/2024']}" />
        <Text testId="out">{dp.value}</Text>
      </Fragment>
    `);
...
test.describe("Api", () => {
  test("setValue updates the input", async ({ page, initTestBed }) => {
    await initTestBed(`
      <Fragment>
        <DatePickerNew id="dp" testId="dp" dateFormat="MM/dd/yyyy" initialValue="05/25/2024" />
  ...
  test("getValue returns the current value", async ({ page, initTestBed }) => {
    await initTestBed(`
      <Fragment>
        <DatePickerNew id="dp" dateFormat="MM/dd/yyyy" initialValue="05/25/2024" />
  ...
  test("bindTo syncs $data and value", async ({ page, initTestBed }) => {
    await initTestBed(`
      <Form hideButtonRow="true">
        <DatePickerNew id="bound" bindTo="dateValue" dateFormat="MM/dd/yyyy" />
  ...
  test("submits its bound value inside a Form", async ({ page, initTestBed }) => {
    const { testStateDriver } = await initTestBed(`
      <Form onSubmit="(data) => testState = data.testField" data="{{ testField: '05/25/2024' }}">
        <DatePickerNew label="Choose Date" bindTo="testField" dateFormat="MM/dd/yyyy" />
      </Form>
    `);
...
test.describe("Layout & Sizing", () => {
  test("full-width fills its container", async ({ page, initTestBed }) => {
    await initTestBed(`
      <Stack width="400px">
        <DatePickerNew testId="dp" width="100%" />
      </Stack>
    `);
  ...
  test("the calendar popup is not clipped by a narrow control", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" width="100%" dateFormat="MM/dd/yyyy" initialValue="05/15/2024" />`);
    await openCalendar(page);
...
test.describe("Behaviors and Parts", () => {
  test("requireLabelMode='markRequired' shows an asterisk for required fields", async ({
    ...
    await initTestBed(`
      <Form>
        <DatePickerNew label="Birth Date" required="true" requireLabelMode="markRequired" bindTo="birthDate" />
      </Form>
    `);
  ...
  test("requireLabelMode='markOptional' shows an optional tag for optional fields", async ({
    ...
    await initTestBed(`
      <Form>
        <DatePickerNew label="Birth Date" required="false" requireLabelMode="markOptional" bindTo="birthDate" />
      </Form>
    `);
    const label = page.getByText("Birth Date");
  ...
  test("requireLabelMode='markBoth' marks required and optional fields distinctly", async ({
    ...
    await initTestBed(`
      <Form>
        <DatePickerNew label="Required Field" required="true" requireLabelMode="markBoth" bindTo="f1" />
        <DatePickerNew label="Optional Field" required="false" requireLabelMode="markBoth" bindTo="f2" />
      </Form>
    `);
  ...
  test("input inherits the Form itemRequireLabelMode when not specified", async ({
    ...
    await initTestBed(`
      <Form itemRequireLabelMode="markBoth">
        <DatePickerNew label="Required Field" required="true" bindTo="f1" />
        <DatePickerNew label="Optional Field" required="false" bindTo="f2" />
      </Form>
    `);
    await expect(page.getByText("Required Field")).toContainText("*");
    await expect(page.getByText("Optional Field")).toContainText("(Optional)");
...
test.describe("Validation Feedback", () => {
  ...
  test("concise feedback prop on the input overrides the Form default", async ({
    ...
    await initTestBed(`
      <Form verboseValidationFeedback="{true}">
        <DatePickerNew testId="dp" bindTo="input" verboseValidationFeedback="{false}" required="{true}" />
        <Button testId="submit" type="submit">Submit</Button>
      </Form>
    `);
    await page.getByTestId("submit").click();
  ...
  test("does not duplicate the label when inside a Form", async ({ page, initTestBed }) => {
    await initTestBed(`
      <Form>
        <DatePickerNew testId="dp" label="Select date" labelPosition="top" />
      </Form>
    `);
...
test.describe("Range Mode Features", () => {
  ...
  test("range mode shows prev and next chevrons", async ({ page, initTestBed }) => {
    await initTestBed(`<DatePickerNew testId="dp" mode="range" inline />`);
  ...
  test("hover preview shows a background on intermediate dates", async ({ page, initTestBed }) => {
    await initTestBed(
      `<DatePickerNew testId="dp" mode="range" dateFormat="MM/dd/yyyy" initialValue="{{ from: '05/01/2024', to: '05/01/2024' }}" />`,
      {
        testThemeVars: {
          "backgroundColor-day-DatePickerNew--rangeMiddle": "rgb(230, 240, 255)",
        },
