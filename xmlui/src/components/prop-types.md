...
| `List` ✅ | `groupBy`, `orderBy`, `syncWithVar` | `any` / `string` | `groupBy` can be a field name or function; `orderBy` and `syncWithVar` are strings. |
...
| `RadioGroup` ✅ | `initialValue` | `string` | Radio option values are string-backed. |
...
| `Slider` ✅ | `initialValue` | `any` | Numeric slider value, but range sliders may need array support. |
...
| `Table` ✅ | `syncWithAppState` | `string` | Description points at an `AppState` instance. |
