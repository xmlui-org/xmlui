...
function extractFromThemeWrapper(
  ...
  themeNode.children?.forEach((child) => {
    ...
    // Also preserve Fragments that have lifecycle events (onInit/onCleanup), since unwrapping
    // them would silently discard those events.
    if (child.type === "Fragment" && child.children && child.when === undefined && !child.events?.init && !child.events?.cleanup) {
