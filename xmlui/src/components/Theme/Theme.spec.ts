...
test.describe("applyIf Edge Cases", () => {
  ...
  test("empty Theme tag without attributes does not create wrapper", async ({ page, initTestBed }) => {
    await initTestBed(`
      <App>
        <VStack testId="control" gap="20px">
          <Button testId="control-button">Control</Button>
        </VStack>
        <Theme>
    ...
    // The empty Theme should not introduce an extra wrapper between siblings.
    const sameParentAsControl = await page.getByTestId("container").evaluate((el) => {
      const control = document.querySelector('[data-testid="control"]');
      return el.parentElement === control?.parentElement;
    });
    expect(sameParentAsControl).toBe(true);
  });
