...
export const toneSwitchRenderer = wrapRuntimeComponent({
  name: COMP,
  metadata: ToneSwitchMd as ComponentMetadata,
  renderer: ({ adapter }) => (
    <ToneSwitch
      {...adapter.rootAttrs()}
      iconLight={adapter.stringProp("iconLight", defaultProps.iconLight)}
      iconDark={adapter.stringProp("iconDark", defaultProps.iconDark)}
      onDidChange={(tone) => {
