import { type Dispatch, useEffect, useLayoutEffect, useMemo, useRef, useState } from "react";
...
export function useValidation(
  ...
  const stableOnValidate = useMemo<ValidateEventHandler>(
    () => onValidate === undefined ? undefined : ((value) => onValidateRef.current?.(value)) as ValidateEventHandler,
  ...
  useLayoutEffect(
    ...
    [bindTo, value, dispatch, stableOnValidate, validations],
  );
  ...
  useEffect(
    ...
    [bindTo, value, dispatch, stableOnValidate, throttledAsyncValidate, validations],
  );
}
