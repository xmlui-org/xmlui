...
import { Fragment, cloneElement, forwardRef, useCallback, useEffect, useId, useLayoutEffect, useMemo, useRef, useContext } from "react";
...
export const ValidationWrapper = forwardRef<HTMLElement, ValidationWrapperProps>(function ValidationWrapper({
  ...
  const animateContainerRef = useRef<HTMLDivElement | null>(null);
  const previousHelperHeightRef = useRef(0);
  useLayoutEffect(() => {
    const node = animateContainerRef.current;
    if (!node) return;

    const previousHeight = previousHelperHeightRef.current;
    const nextHeight = node.getBoundingClientRect().height;
    previousHelperHeightRef.current = nextHeight;

    if (previousHeight === nextHeight || window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      return;
    }
    ...
    animation.addEventListener(
      "finish",
      () => {
        if (animateContainerRef.current === node) {
  ...
  }, [isHelperTextShown, validationResult]);
  ...
  const validationResultDisplay =
    effectiveVerboseValidationFeedback === false ? null : (
      <Part partId="validationStatusIndicator">
        <div ref={animateContainerRef} {...validationAttributes} className={styles.helperTextContainer}>
          {isHelperTextShown &&
  ...
  return (
    <div ref={wrapperRef} className={styles.validationWrapper}>
