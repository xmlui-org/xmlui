...
test.describe("Basic Functionality", () => {
  ...
  test.describe("stretch property", () => {
    test("stretch='true' makes splitter take full height of parent", async ({ initTestBed, page }) => {
      await initTestBed(`
        <VStack height="400px" testId="container">
          <Splitter stretch="true" testId="splitter">
            <Stack backgroundColor="lightblue" height="100%" testId="primary"/>
            <Stack backgroundColor="darksalmon" height="100%" testId="secondary"/>
          </Splitter>
        </VStack>
      ...
      // Splitter should take the full height of its parent container
      const tolerance = 5;
      expect(Math.abs(splitterBounds.height - containerBounds.height)).toBeLessThan(tolerance);
    });

    test("stretch='false' does not stretch splitter", async ({ initTestBed, page }) => {
      await initTestBed(`
        <VStack height="400px" testId="container">
          <Splitter stretch="false" height="200px" testId="splitter">
            <Stack backgroundColor="lightblue" height="100%" testId="primary"/>
            <Stack backgroundColor="darksalmon" height="100%" testId="secondary"/>
          </Splitter>
        </VStack>
      ...
      // Splitter should maintain its explicit height
      expect(splitterBounds.height).toBe(200);
    });
    ...
    test("stretch works with horizontal orientation", async ({ initTestBed, page }) => {
      await initTestBed(`
        <VStack height="400px" testId="container">
          <Splitter stretch="true" orientation="horizontal" testId="splitter">
            <Stack backgroundColor="lightblue" height="100%" testId="primary"/>
            <Stack backgroundColor="darksalmon" height="100%" testId="secondary"/>
          </Splitter>
        </VStack>
      ...
      // Stretch should work regardless of orientation
      const tolerance = 5;
      expect(Math.abs(splitterBounds.height - containerBounds.height)).toBeLessThan(tolerance);
    });
  });
