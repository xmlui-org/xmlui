...
export type ValueTypeSource = "explicit" | "inferred" | "fallback";

export type ValueTypeDiagnosticCode =
  | "empty-type"
  | "invalid-type-value"
  | "unknown-type"
  | "malformed-arguments"
...
export type ValueTypeDiagnostic = {
  code: ValueTypeDiagnosticCode;
  message: string;
  input?: unknown;
  argument?: string;
};
...
export type NormalizeValueTypeResult = {
...
const SUPPORTED_VALUE_TYPES = new Set<string>(VALUE_TYPE_NAMES);
const DATE_TIME_STYLES = new Set(["short", "medium", "long", "full"]);

export function isValueTypeName(value: string): value is ValueTypeName {
  return SUPPORTED_VALUE_TYPES.has(value);
}

export function normalizeValueType(
  rawType: unknown,
  rawTypeOptions?: unknown,
  source: Exclude<ValueTypeSource, "fallback"> = "explicit",
): NormalizeValueTypeResult {
  const diagnostics: ValueTypeDiagnostic[] = [];
  let typeOptionsEvaluated = false;
  let typeOptions: Pick<NormalizedValueType, "options"> | undefined;
  ...
  const fallback = (): NormalizedValueType => ({
    name: "text",
    source: "fallback",
    ...(getTypeOptions() ?? {}),
  });
  ...
  if (typeof rawType !== "string") {
    diagnostics.push({
      ...
      message: "Value type must be a string.",
      input: rawType,
    });
  ...
  if (!isValueTypeName(name)) {
    diagnostics.push({
      code: "unknown-type",
      message: `Unknown value type '${parsed.name}'.`,
      input: rawType,
    });
    return { type: fallback(), diagnostics };
  }

  const normalized: NormalizedValueType = {
    name,
    source,
  };
...
function normalizedTypeOptions(
  rawTypeOptions: unknown,
  diagnostics: ValueTypeDiagnostic[],
): Pick<NormalizedValueType, "options"> | undefined {
  ...
  if (
    ...
    diagnostics.push({
      ...
      message: "Value typeOptions must be an object.",
...
function parseTypeExpression(input: string):
  ...
  if (!match) {
    return { ok: false, message: "Value type must use the form 'name' or 'name(args)'." };
  }
  ...
  if (rawArguments.trim() === "") {
    return { ok: false, message: "Value type argument list cannot be empty." };
  }
  if (/[()]/.test(rawArguments)) {
    return { ok: false, message: "Value type arguments cannot contain nested parentheses." };
  }

  const parts = rawArguments.split(",");
  if (parts.some((part) => part.trim() === "")) {
    return { ok: false, message: "Value type arguments cannot be empty." };
  }
  ...
  if (hasNamed && hasPositional) {
    return {
      ok: false,
      message: "Value type arguments cannot mix positional and named syntax.",
    };
  }
      ...
      if (arg.kind === "named") {
        if (Object.prototype.hasOwnProperty.call(options, arg.name)) {
          diagnostics.push({
            ...
            message: `Duplicate value type argument '${arg.name}'.`,
            input,
            argument: arg.name,
          });
          continue;
  ...
  switch (typeName) {
    ...
    default:
      diagnostics.push({
        ...
        message: `Value type '${typeName}' does not support positional arguments.`,
        input,
      });
      return { args: normalizedArgs, options, diagnostics, shouldFallback: true };
  }
...
function invalidArgumentCount(
  ...
  diagnostics.push({
    code: "invalid-argument",
    message: `Invalid argument count for value type '${typeName}'.`,
    input,
  });
