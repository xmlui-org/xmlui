...
test.describe("Theme Vars", () => {
  ...
  test.skip("component maintains its height in a vertical container", async ({
    page,
    initTestBed,
  }) => {
    // TODO: review these Copilot-created tests
...
test.describe("Accessibility", () => {
  ...
  test.skip("component doesn't interfere with keyboard navigation", async ({
    page,
    initTestBed,
  }) => {
    // TODO: review these Copilot-created tests
...
test.skip("component works with both orientations in the same layout", async ({
  page,
  initTestBed,
}) => {
  // TODO: review these Copilot-created tests
...
// - If a width % is set for at least two items including the separator,
//   it will occupy all of the space given to it in the width property (makes it too wide)
test.fixme(`vertical in FlowLayout`, async ({ initTestBed, page }) => {
  ...
  const { height } = await getBounds(separator);
  expect(height).toEqualWithTolerance(expectedSizePx);
});

// TODO: Add more FlowLayout integration tests
