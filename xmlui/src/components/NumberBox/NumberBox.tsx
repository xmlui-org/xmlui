...
function applyStep(valueStrRep: string, step: number, min: number, max: number, integersOnly: boolean) {
...
// ============================================================================
// XMLUI NumberBox definition

/**
 * A \`NumberBox\` component allows users to input numeric values: either integer
 * or floating point numbers. It also accepts empty values, where the stored value
 * will be of type \`null\`.
 *
 * The `NumberBox` is an input control component and is often used in forms.
 * See the [Using Forms](/learning/using-components/forms/) guide for details.
 */
export interface NumberBoxComponentDef extends ComponentDef<"NumberBox"> {
  props: {
    /** @descriptionRef */
    placeholder?: string;
    /** @descriptionRef */
  ...
  api: {
    ...
    value: number | string | null;
    /**
     * Invoke this method to set the focus to the component.
     */
    focus: () => void;
    /** @descriptionRef */
...
export const NumberBoxMd: ComponentDescriptor<NumberBoxComponentDef> = {
  ...
  props: {
    ...
    zeroOrPositive: desc("Indicates if the component accepts only positive numbers"),
    minValue: desc("Lower boundary of input values (inclusive)"),
    maxValue: desc("Upper boundary of input values (inclusive)"),
  },
  events: inputComponentEventDescriptors,
  themeVars: parseScssVar(styles.themeVars),
};

export const numberBoxComponentRenderer = createComponentRenderer<NumberBoxComponentDef>(
  "NumberBox",
