...
import type { IconRegistry } from "@components/IconProvider";
...
export function useCustomSvgIconRenderer(resourceUrl?: string){
  ...
  const iconRenderer = useCallback(({style, className}: any)=>{
    if(!customSvg){
      return null;
    }
    ...
    return <svg {...safeAttributes} style={style} className={className}>
      <use href={`#${name}`} />
    </svg>
  }, [customSvg]);
