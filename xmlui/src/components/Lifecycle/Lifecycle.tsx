...
export const lifecycleRenderer = wrapRuntimeComponent({
  name: COMP,
  metadata: LifecycleMd as ComponentMetadata,
  renderer: ({ adapter }) => {
    useEffect(() => {
      void adapter.event("mount")();
      return () => {
        void adapter.event("unmount")();
      };
    }, []);

    return (
      <Lifecycle
        keyValue={adapter.prop("keyValue")}
        onMount={() => adapter.event("mount")()}
        onUnmount={() => void adapter.event("unmount")()}
        onError={(payload) => void adapter.event("error")(payload)}
      />
    );
  },
});
