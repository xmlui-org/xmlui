...
export const formValidatorRenderer = wrapRuntimeComponent({
  name: COMP,
  metadata: FormValidatorMd,
  renderer({ adapter }) {
    return (
      <FormValidatorReact
        bindTo={adapter.stringProp("bindTo")}
        severity={(adapter.stringProp("severity", "error") as "error" | "warning") ?? "error"}
        onValidate={adapter.node.events.validate ? (data) => adapter.event("validate")(data) : undefined}
      />
    );
  },
});
