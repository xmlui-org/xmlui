...
import { TableOfContents2, defaultProps } from "./TableOfContents2Native";
import { useIndexerContext } from "../App/IndexerContext";
import { createMetadata, dContextMenu } from "../metadata-helpers";

const COMP = "TableOfContents2";
const COMP_CHILD = "TableOfContents2Item";

export const TableOfContents2Md = createMetadata({
  status: "stable",
  description:
    "`TableOfContents2` collects [Heading](/components/Heading) and " +
...
function IndexAwareTableOfContents2(props: React.ComponentProps<typeof TableOfContents2>) {
  const { indexing } = useIndexerContext();
  if (indexing) {
    return null;
  }
  return <TableOfContents2 {...props} />;
}

export const tableOfContents2Renderer = createComponentRenderer(
  COMP,
  TableOfContents2Md,
  ({ className, node, extractValue, lookupEventHandler }) => {
    return (
      <IndexAwareTableOfContents2
        className={className}
