...
export const slotRenderer: XmluiBuiltInRenderer = ({ context, node, scope }) => {
  ...
  if (!fragment) {
    return createElement(Fragment, null, context.renderChildren(node.children, scope));
  }
  ...
  return createElement(
    Fragment,
    null,
    context.renderChildren(fragment.children, createSlotScope(fragment.scope, contextValues)),
  );
};
