...
export const FileUploadDropZone = memo(forwardRef(function FileUploadDropZone(
  ...
  const valueRef = useRef<File[]>([]);

  const publishApi = useCallback((openApi: () => void) => {
    registerComponentApi?.({
      open: openApi,
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        valueRef.current = acceptedFiles;
        updateState?.({
          value: acceptedFiles,
        });
  ...
  const rootRef = getComposedRef(ref as React.Ref<HTMLDivElement>, forwardedRef);
  return (
    <div {...rootProps} data-drop-enabled={!disabled} ref={rootRef}>
      <input {...getInputProps()} />
      {children}
      {!children && !isDragActive && (
        <div className={classnames(styles.dropPlaceholder)} data-part-id="dropPlaceholder" data-xmlui-part="dropPlaceholder">
          <ThemedIcon name={icon} data-icon={icon}></ThemedIcon>
          {text}
        </div>
      )}
      {isDragActive && isDragAccept && (
        <div className={classnames(styles.dropPlaceholder, styles.onDragActive)} data-part-id="dropPlaceholder" data-xmlui-part="dropPlaceholder">
          <ThemedIcon name={icon} data-icon={icon}></ThemedIcon>
          {text}
        </div>
      )}
