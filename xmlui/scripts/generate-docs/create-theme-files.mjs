...
async function generateThemeFiles() {
  ...
  try {
    ...
    if (exportedThemeCount > 0) {
      ...
      logger.info(
        `Exported ${exportedThemeCount} theme files with a total of ${totalThemeVars} theme variables`,
      );
      logger.info(
        `Average of ${Math.round(totalThemeVars / exportedThemeCount)} theme variables per theme file`,
      );
    }
