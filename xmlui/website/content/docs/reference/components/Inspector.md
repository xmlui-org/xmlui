...
**Key features:**
- **One-line integration**: Replace manual Icon + ModalDialog + IFrame boilerplate with a single `<Inspector />` tag
- **Trace visualization**: View interaction timelines, API call/response correlation, state change tracking, and handler timing
- **Export support**: Export traces as HTML, Markdown, or JSON for analysis
- **Automatic trace filtering**: Inspector interactions are excluded from the app's own traces
...
2. **`xmlui-parser.es.js`** — the standalone XMLUI parser (from the XMLUI build at `dist/inspector/`)
