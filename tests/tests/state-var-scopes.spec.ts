...
test("vars shadowing works", async ({ page }) => {
  await initApp(page, {
    entryPoint: `
...
test("inner input is available in the file (implicit containers because of vars)", async ({ page }) => {
  await initApp(page, {
    entryPoint: `
      <Fragment>
          <Stack var.x="x in outer stack" var.y="y in outer stack">
...
test("inner datasource is available in the file (implicit containers because of vars)", async ({ page }) => {
  await initApp(page, {
    entryPoint: `
      <Fragment>
          <Stack var.x="x in outer stack" var.y="y in outer stack">
