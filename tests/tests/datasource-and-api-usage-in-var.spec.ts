...
test("regression: datasource usage in compound component var", async ({ page }) => {
  await initApp(page, {
    entryPoint: `
      <TestComp/>
    `,
...
test("regression: api usage in compound component var", async ({ page }) => {
  await initApp(page, {
    entryPoint: `
      <TestComp/>
    `,
