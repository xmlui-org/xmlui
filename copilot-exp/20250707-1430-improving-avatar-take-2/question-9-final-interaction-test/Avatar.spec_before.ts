...
test("avatar is keyboard accessible when clickable", async ({ initTestBed, createAvatarDriver }) => {
  ...
  const activationData = await driver.component.evaluate(() => (window as any).keyboardActivated);
...
test("click event provides correct event data", async ({ initTestBed, createAvatarDriver }) => {
  ...
  // Create a function that captures click events
  await initTestBed(`
    <Avatar 
      name="${TEST_NAME}" 
      url="${TEST_URL}"
      onClick={(e) => {
        // Store event data in a global variable for testing
        window.lastAvatarClick = {
          name: '${TEST_NAME}',
          url: '${TEST_URL}',
          hasImage: true,
          eventType: e.type
  ...
  // Verify the event was captured with correct data
  const clickData = await driver.component.evaluate(() => (window as any).lastAvatarClick);
...
test("avatar applies clickable styling when onClick provided", async ({ initTestBed, createAvatarDriver }) => {
  const mockClickHandler = "console.log('clicked')";
  const TEST_NAME = "Clickable User";

  await initTestBed(`<Avatar name="${TEST_NAME}" onClick={${mockClickHandler}}/>`, {});
  const driver = await createAvatarDriver();
...
test("avatar hover state works correctly", async ({ initTestBed, createAvatarDriver }) => {
  ...
  const mockClickHandler = "console.log('hovered and clicked')";
