...
<Callout type="info" emoji="💡">
  **XMLUI apps** are declarative. You define an app with an XMLUI (XML) markup. [Learn more!](./learning/xmlui-markup)
</Callout>
...
```
<Callout type="info" emoji="💡">
  **Components** are the basic building blocks of XMLUI apps. A component is an XML element (tag) in the markup. [Learn more!](./learning/xmlui-markup/#tags)
</Callout>
...
<Callout type="info" emoji="💡">
  When properties cannot be represented with a string, you nest them into their related component with the **`<property>` tag**. [Learn more!](./learning/xmlui-markup/#properties)
</Callout>
...
<Callout type="info" emoji="💡">
  ...
  Defining **event handlers** with expressions or short scripts (with statements, loops, and other constructs) can change how components respond to particular events. [Learn more!](./learning/xmlui-markup/#event-handlers)
</Callout>
...
<Callout type="info" emoji="💡">
  Components may have an **ID**. Other components can utilize this ID to refer to **exposed component members** (values and methods).
</Callout>
...
```xmlui /when="{useCompression.value}"/
...
<Callout type="info" emoji="💡">
  Components can **invoke API endpoints that mutate data**. You can use the **`<event>` tag** to bind the invocation of a particular endpoint to a component's event handler. An **APICall component** can declare the details of the invocation.
</Callout>

```xmlui copy
<Button label="Delete completed tasks">
  <event name="click">
    <APICall url="/api/tasks/completed" method="delete" />
  </event>
</Button>
```
...
```xmlui copy filename="Main.xmlui (markup)" /calcButton/
...
<div style={{display: "flex", flexDirection: "row"}}>
  <div style={{width: "50%", paddingRight: "0.25rem"}}>
```xmlui copy filename="Main.xmlui (markup)" /handleOnClick/
<Button
  label="Calculate delay"
...
```xmlui copy /{tasks.length}/ /initialValue="{customer.name}"/ /delay="{calculatedDelay * (1 + reservePercentage * 0.01)} minutes"/
...
When you click the button, it increments only the `count` variable. However, as `countTimes` is a reactive variable with `count` as a dependency, updating `count` triggers the re-evaluation of `countTimes3`. Thus, the second text will show three times the value of the first.

<Callout type="info" emoji="💡">
You can use **local variables** within (and only within) function declarations. The **`let` keyword** allows using a mutable variable; **`const`** declares an immutable one (following the JavaScript semantics). In contrast to `var`, variables declared with `let` and `const` are not reactive.
</Callout>
...
```xmlui copy {2-4} /property name="itemTemplate"/
...
```
  ...
  <div style={{width: "50%", paddingLeft: "0.25rem"}}>
```xmlui copy filename="InfoCard.xmlui (reusable component)"
...
<Callout emoji="💡">
  **You do not need to build an app**. However, the framework offers several build methods that **optimize the static file structure for size and startup performance** if you need this feature.
</Callout>
