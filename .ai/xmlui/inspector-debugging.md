...
| `"debug-source"`                                                                                  | compiled script executors            | Deduplicated metadata for compiled binding/event artifacts when `xsVerbose` is on and the app is dev-served (source maps); payload includes `target`, `sourceUrl`, `originalFile`, `sourceRange`, `artifactId`, and `sourcemapMode` without full source text |
| `"compile"`                                                                                       | compile diagnostics                  | A script block that is not running as compiled JavaScript; payload includes `code` (`compile-unsupported-node`, `compile-unserializable-literal`, `compile-runtime-fallback`, `compile-source-unavailable`), `severity`, `sourceId`, `message`, and — where known — `construct`, `line`, `column`. `xmluiConfig.reportCompileFallbacks` additionally prints it to the console |
