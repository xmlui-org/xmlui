...
| G4 | **`when=false` with `init` event runs once** — a component fires its `onInit` handler exactly once even when `when` is initially false, enabling one-time initialization that runs before the component becomes visible | Medium | Extend the existing "when and lifecycle events" subsection in `visibility.md` with a paragraph and code example |
| G5 | **`displayWhen` vs `when` for form fields** — fields hidden by `displayWhen` remain registered and are still validated; fields hidden by `when` are fully removed and skipped by validation | High — silent form bugs | In `visibility.md`, where `when` is explained, immediately introduce `displayWhen` as its counterpart and add a comparison table covering DOM presence, form field registration, state preservation, and when to use each. Add a cross-reference callout in `forms.md` |
...
| G13 | **`$params` (plural) in `APICall`** — `$param` (first argument to `execute()`) is in `context-variables.md`; `$params` (all arguments as an array, accessed as `$params[0]`, `$params[1]`, etc.) is not | Low | Add `$params` row to `context-variables.md` in the Event handlers section |
