...
- `Main.xmlui` may also contain top-level `<Component>` declarations before or after the single app root. These inline entrypoint components are reusable by the app in the same file. The order is insignificant; choose app-first or component-first examples according to the documentation focus.
...
Context variables are `# XMLUI Markup Patterns
...
-prefixed variables injected dynamically by components into their template slots or event handlers. They cannot be declared by users via attributes.

Previously, variables like `$item`, `$data`, or `$routeParams` were treated as hardcoded framework features. Now, **there are no hardcoded context variables** (with the sole exception of the global `$context`). 

Instead, context variables are strictly scoped and explicit:
1. **Component-driven**: A component like `List` decides to inject `$item` into its children. A `Form` injects `$data`.
2. **Metadata Contract**: The variables a component injects *must* be declared in its metadata (`childInjectedVars` for templates, or `injectedVars` for events). 
3. **Lexical Scoping Engine**: The framework's dependency optimizer scans expressions and automatically strips any `# XMLUI Markup Patterns
...
-prefixed variables that the wrapping component hasn't explicitly declared in its metadata. This prevents unnecessary re-renders when parent context variables change.

For example, when using a component, check its specific documentation to see which `# XMLUI Markup Patterns
...
**`Globals.xs`** is special — every variable and function declared there becomes a global visible everywhere in the app. `Main.xmlui.xs` declarations are local to the Main component, just like any other code-behind file.
 ...
 variables it injects into its slots.
