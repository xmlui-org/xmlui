...
describe("Xmlui transform - entity escape", () => {
  it("&amp;", () => {
    ...
    expect(cd.children![0].props!.value).toEqual("&");
  });
  ...
  it("&nbsp;", () => {
    const cd = transformSource("<Stack>&nbsp;&nbsp;&nbsp;</Stack>") as ComponentDef;
    expect(cd.children![0].props!.value).toEqual("\xa0\xa0\xa0");
  });

  it("combined", () => {
    const cd = transformSource("<Stack>&quot;&amp;abc&gt;&lt;</Stack>") as ComponentDef;
    expect(cd.children![0].props!.value).toEqual('"&abc><');
  });
  ...
  it("attr - &nbsp;", () => {
    const cd = transformSource("<H1 value='&nbsp;&nbsp;&nbsp;' />") as ComponentDef;
    expect(cd.props!.value).toEqual("\xa0\xa0\xa0");
  });
  ...
  it("attr - combined", () => {
    ...
    expect(cd.props!.value).toEqual('"&abc>');
  });
});
