...
export default async function extensionE2eGlobalSetup() {
  if (!extensionPackage || process.env.XMLUI_E2E_WARMUP === "0") {
    return;
  }
  ...
  try {
    await page.addInitScript((packageName) => {
      window.sessionStorage.setItem("__xmluiTestBedSource", "<App />");
      window.sessionStorage.setItem("__xmluiTestBedComponents", "[]");
      window.sessionStorage.setItem("__xmluiTestBedExtensionIds", JSON.stringify([packageName]));
