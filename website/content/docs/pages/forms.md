...
```xmlui-pg name="Forms" display id="forms-b6ec"
<App>
...
```xmlui-pg name="Form Layouts" display id="form-layouts-ea4e"
<App>
  <Form data="{{
      firstname: 'Jake',
      lastname: 'Hard',
...
```xmlui-pg name="Form Layouts 2" display id="form-layouts-eab2"
<App>
  <Form
    data="{{
      firstname: 'Jake',
...
```xmlui-pg name="Form Layouts 3" display id="form-layouts-eab0"
<App>
  <Form
    data="{{
      title: 'Mr.',
...
```xmlui-pg name="Checkbox" display id="checkbox-163e"
<App>
...
```xmlui-pg name="DatePicker" display id="datepicker-1662"
<App>
  <Form
...
```xmlui-pg name="FileInput" display id="fileinput-16a0"
<App>
  <Form
    data="{{ articles: null }}"
    onSubmit="(toSave) => toast.success(JSON.stringify(toSave))">
...
```xmlui-pg name="NumberBox (integers)" display id="numberbox-integers-16e2"
<App>
  <Form
    data="{{ age: 30 }}"
    onSubmit="(toSave) => toast.success(JSON.stringify(toSave))">
...
```xmlui-pg name="NumberBox (floating-point)" display id="numberbox-floating-point-1716"
<App>
  <Form
    data="{{ distance: 192.5 }}"
    onSubmit="(toSave) => toast.success(JSON.stringify(toSave))">
...
```xmlui-pg name="RadioGroup" display id="radiogroup-1744"
<App>
  <Form
    data="{{ title: 'Mr.' }}"
    onSubmit="(toSave) => toast.success(JSON.stringify(toSave))">
...
```xmlui-pg name="Select" display id="select-1792"
<App>
  <Form
    data="{{ size: 'xs' }}"
    onSubmit="(toSave) => toast.success(JSON.stringify(toSave))">
...
```xmlui-pg name="Switch" display id="switch-17d0"
<App>
  <Form
...
```xmlui-pg name="TextBox" display id="textbox-17fe"
<App>
  <Form
    data="{{ name: 'Joe' }}"
    onSubmit="(toSave) => toast.success(JSON.stringify(toSave))">
...
```xmlui-pg name="TextArea" display id="textarea-182c"
<App>
  <Form
...
```xmlui-pg name="Refer to data" display id="refer-to-data-1944"
<App>
...
```xmlui-pg name="Refer to data 2" display id="refer-to-data-1972"
<App>
...
```xmlui-pg name="Refer to data 3" display {9} id="refer-to-data-19a0"
<App>
  <Form
    data="{{
      name: 'John smith',
...
```xmlui-pg name="minLength" display id="validate-data-19ee"
<App>
...
```xmlui-pg name="maxLength" display id="validate-data-1a1c"
<App>
  <Form data="{{ name: 'Billy Bob' }}">
...
```xmlui-pg name="minValue" display id="validate-data-1a4a"
<App>
  <Form data="{{ age: 30 }}">
...
```xmlui-pg name="maxValue" display id="validate-data-1a78"
<App>
  <Form data="{{ age: 30 }}" >
...
```xmlui-pg name="pattern" id="compound-validation-1aa6"
<App>
  <Form data="{{
      mobile: '+13456123456',
...
```xmlui-pg name="regex" display id="validation-specific-severity-1af4"
<App>
...
```xmlui-pg name="Compound validation" display id="validation-specific-messages-1b22"
<App>
...
```xmlui-pg name="Validation-specific severity" display id="server-side-validation-1b50"
<App>
...
```xmlui-pg name="Validation-specific messages" display id="submit-data-1b9e"
<App>
  <Form data="{{ age: 20 }}" >
    <NumberBox
      bindTo="age"
...
```xmlui-pg name="Submit data" display id="submit-data-1bec"
<App>
...
```xmlui-pg name="Submit data 2" display id="submit-data-1c1a"
<App>
  <Form
...
```xmlui-pg name="Custom inputs with FormItem" display id="custom-inputs-with-formitem-1d7a"
<App>
  <Form
