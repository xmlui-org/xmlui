...
**[Free tracing](/docs/guides/wrap-component/free-tracing).** Wrapped components emit semantic trace events (`value:change`, `focus:change`) with no extra code. The `aria-label` prop becomes the component's accessible name in traces -- a screenreader announces "Volume, slider, 50" and the trace records `value:change didChange Slider [Volume] "53"`. This unlocks accessibility, semantic traces that AIs can reason about, and [regression tests](https://github.com/xmlui-org/xmlui-regression) generated from user journeys by [trace-tools](https://github.com/xmlui-org/trace-tools). Export a trace and hand it to an AI -- it doesn't need to read your source code. The trace *is* the conversation.
