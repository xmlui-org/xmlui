...
<App var.selectedTag="all">
  <HStack gap="$space-4" padding="$space-4" alignItems="start">
    ...
    <VStack gap="$space-2">
      ...
      <Text when="{selectedTag === 'all' || selectedTag === 'ui'}">Build a search bar — UI</Text>
      <Text when="{selectedTag === 'all' || selectedTag === 'data'}">Paginate API results — Data</Text>
      <Text when="{selectedTag === 'all' || selectedTag === 'layout'}">Create a responsive grid — Layout</Text>
      <Text when="{selectedTag === 'all' || selectedTag === 'ui'}">Show a confirmation dialog — UI</Text>
      <Text when="{selectedTag === 'all' || selectedTag === 'data'}">Cache API responses — Data</Text>
    </VStack>
...
**Declare the shared variable on the closest common ancestor**: Use `var.name="value"` on the element that contains both siblings — typically `App`, a layout element, or a custom component. Both siblings can then read and write the same variable.

**Writes from one sibling re-render the other automatically**: When the filter panel sets `selectedTag`, the article list's `when` expressions re-evaluate immediately. No event bus or callback wiring is required.

**Use `global.name` to share across the full App subtree**: Declaring `global.hiddenCategories` on `App` makes the variable accessible deep in any nested custom component, not just direct children.
