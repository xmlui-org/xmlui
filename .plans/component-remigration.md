...
| Audit required | Component exists in the rewrite but has not passed the new protected-file audit. | 16 |
| Blocked | Known prerequisite missing before strict migration can finish. | 0 |
| In review | Audit and tests passed; waiting for user approval. | 3 |
| Complete | User approved after audit and verification. | 93 |
...
| ScrollViewer | `ScrollViewer.tsx`, `ScrollViewerReact.tsx`, `Scroller.tsx`, `ScrollViewer.defaults.ts`, `ScrollViewer.module.scss`, `ScrollViewer.md`, `ScrollViewer.spec.ts`; added `ScrollViewer.foundation.spec.ts` | layout, Bookmark, original overlay scrollbar dependencies | Complete | User approved ScrollViewer as complete after protected-file audit, metadata verification, focused E2E coverage, live DOM parity checks, and follow-up height/fade regressions. Strict copy migrated from original. `node xmlui/scripts/verify-protected-component-copy.mjs ScrollViewer` passed (`ScrollViewer.tsx` entry-adapted; copied React/Scroller/SCSS/defaults/docs/spec identical). Runtime adapter appended below copied entry; obsolete sidecar renderer removed and registries import the provider-style entry. Added original `overlayscrollbars@2.13.0` and `overlayscrollbars-react@0.5.6` dependencies. Host fixes stayed outside copied source: shared `useIsTouchDevice` shim, root-attribute bridge for copied header/footer wrapper mode, an empty-viewer min-height host fallback so copied empty-content coverage remains visible, plain-scroller width/height filtering so copied inline `width: 100%; height: 100%` keeps legacy precedence over authored layout dimensions while preserving background/layout styles, and overlay container theme-class passthrough so fade overlays inherit the original `height-fade-Scroller` and `backgroundColor-fade-Scroller` variables. Verification passed: `npm --prefix xmlui run check:metadata`; focused failure slice `XMLUI_E2E_DEV_PORT=5442 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts --grep "renders scrollable content and header/footer templates|handles empty content|Header and Footer Templates" --workers=1` passed 12/12; final focused batch `XMLUI_E2E_DEV_PORT=5445 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/SkipLink/SkipLink.spec.ts xmlui/src/components/SelectionStore/SelectionStore.foundation.spec.ts xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 60/60; follow-up regression `npx playwright test xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts --reporter=line` passed 4/4, and live DOM probes measured the user's normal sample at `660px` high on both old `http://localhost:5173/` and rewrite `http://localhost:5174/`, then measured overlay fades on the rewrite at `64px` high with gradient backgrounds after scrolling. |
...
| SelectionStore | `SelectionStore.tsx`, `SelectionStoreReact.tsx`, `SelectionStore.defaults.ts`, `SelectionStore.md`; added `SelectionStore.foundation.spec.ts` | Table/List/Tree | Complete | User approved SelectionStore as complete after protected-file audit, metadata verification, and focused E2E coverage. Strict copy migrated from original. `node xmlui/scripts/verify-protected-component-copy.mjs SelectionStore` passed (`SelectionStore.tsx` entry-adapted; copied React/defaults/docs identical). Runtime adapter appended below copied entry and preserves local selected-items state plus the legacy component API value bridge needed by existing SelectionStore foundation coverage. Verification passed: `npm --prefix xmlui run check:metadata`; final focused batch `XMLUI_E2E_DEV_PORT=5445 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/SkipLink/SkipLink.spec.ts xmlui/src/components/SelectionStore/SelectionStore.foundation.spec.ts xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 60/60. |
| SkipLink | `SkipLink.tsx`, `SkipLinkReact.tsx`, `SkipLink.defaults.ts`, `SkipLink.md`, `SkipLink.spec.ts` | accessibility | Complete | User approved SkipLink as complete after protected-file audit, metadata verification, and focused E2E coverage. Strict copy migrated from original, replacing the interim rewrite-only module SCSS/renderer implementation with copied `SkipLinkReact.tsx` and defaults. `node xmlui/scripts/verify-protected-component-copy.mjs SkipLink` passed (`SkipLink.tsx` entry-adapted; copied React/defaults/docs/spec identical). Runtime adapter appended below copied entry; obsolete sidecar renderer and non-original module SCSS removed; registries import the provider-style entry. Verification passed: `npm --prefix xmlui run check:metadata`; final focused batch `XMLUI_E2E_DEV_PORT=5445 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/SkipLink/SkipLink.spec.ts xmlui/src/components/SelectionStore/SelectionStore.foundation.spec.ts xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 60/60. |
...
| Splitter | `Splitter.tsx`, `SplitterReact.tsx`, `Splitter.module.scss`, `Splitter.defaults.ts`, `Splitter.md`, `HSplitter.md`, `VSplitter.md`, `Splitter.spec.ts`, `HSplitter.spec.ts`, `VSplitter.spec.ts`, `utils.ts`; added `Splitter.foundation.spec.ts` | layout/star sizing | Complete | User approved Splitter, HSplitter, and VSplitter as complete after protected-file audit and verification. Strict copy migrated from original. Runtime adapters for Splitter/HSplitter/VSplitter now live below the copied `Splitter.tsx` entry and obsolete `Splitter.renderer.tsx` was removed. Host-side fixes outside protected source filter invisible conditional children before passing them to the copied Splitter, preserve original HSplitter/VSplitter orientation despite authored `orientation`, strip host-generated root flex direction that conflicts with copied CSS, stabilize resize event callbacks with the shared `useEvent` helper so a state-updating `onResize` handler does not retrigger the copied initialization effect, update the shared Splitter test driver for CSS-module resizer classes, and wait one animation frame in `getBounds` so ResizeObserver-driven layout assertions measure after browser layout settles. Added foundation coverage for the user's `<Splitter onResize="counter++">` sample, which must render `Resize event called 1 number of times` after initial load. `node xmlui/scripts/verify-protected-component-copy.mjs Splitter` passed (`Splitter.tsx` entry-adapted; copied React/SCSS/defaults/docs/spec/helper files identical). Verification passed: `npm --prefix xmlui run check:metadata`; focused Splitter batch `XMLUI_E2E_DEV_PORT=5455 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/Splitter/Splitter.spec.ts xmlui/src/components/Splitter/HSplitter.spec.ts xmlui/src/components/Splitter/VSplitter.spec.ts xmlui/src/components/Splitter/Splitter.foundation.spec.ts --workers=1` passed 74/74; earlier migration batch `XMLUI_E2E_DEV_PORT=5452 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/Slot/Slot.spec.ts xmlui/src/components/Slot/Slot.foundation.spec.ts xmlui/src/components/Part/Part.spec.ts xmlui/src/components/Splitter/Splitter.spec.ts xmlui/src/components/Splitter/HSplitter.spec.ts xmlui/src/components/Splitter/VSplitter.spec.ts xmlui/src/components/ProfileMenu/ProfileMenu.foundation.spec.ts --workers=1` passed 87/87. |
...
| StickyBox | `StickyBox.tsx`, `StickyBoxReact.tsx`, `StickyBox.defaults.ts`, `StickyBox.module.scss`, `StickyBox.md`; updated `StickySection.foundation.spec.ts` | scroll containers, original sticky dependency | Complete | User approved StickyBox as complete after protected-file audit, metadata verification, focused E2E coverage, and the Vite consumer compatibility fix. Strict copy migrated from original. `node xmlui/scripts/verify-protected-component-copy.mjs StickyBox` passed (`StickyBox.tsx` entry-adapted; copied React/SCSS/defaults/docs identical). Runtime adapter appended below copied entry; obsolete sidecar renderer removed and registries import the provider-style entry. Added original `react-sticky-el@^2.1.1` dependency and restored shared `useRealBackground` host shim outside copied source. Follow-up Vite consumer compatibility fix added `src/compat/reactStickyEl.ts` and imports `RenderPropSticky` through it, because browser Vite served `react-sticky-el`'s CommonJS files without synthesized exports and then failed on raw `exports`; the shim now provides the render-prop bridge locally without importing the CommonJS package at runtime. The unprotected Sticky foundation test was realigned from the interim native-sticky shim assertion to the migrated original behavior by asserting rendered StickyBox content and keeping the existing StickySection mutation coverage. Verification passed: `npm --prefix xmlui run check:metadata`; `XMLUI_E2E_DEV_PORT=5470 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 3/3; final focused batch `XMLUI_E2E_DEV_PORT=5445 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/SkipLink/SkipLink.spec.ts xmlui/src/components/SelectionStore/SelectionStore.foundation.spec.ts xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 60/60. |
...
| Tabs | `Tabs.tsx`, `TabItem.tsx`, `TabsReact.tsx`, `TabItemReact.tsx`, `TabContext.tsx`, `Tabs.module.scss`, `Tabs.defaults.ts`, `Tabs.md`, `TabItem.md`, `Tabs.spec.ts` | Radix Tabs, template children, layout | In review | Strict copy migrated from original. `node xmlui/scripts/verify-protected-component-copy.mjs Tabs` passed (`Tabs.tsx` and `TabItem.tsx` entry-adapted; copied React/context/SCSS/defaults/docs/spec identical). Runtime adapters for Tabs and TabItem now live below copied entry files; obsolete `Tabs.renderer.tsx` removed; registries import the provider-style entries. Added the original `@radix-ui/react-tabs@1.1.13` dependency. Verification passed: `npm --prefix xmlui run check:metadata`; final combined copied E2E batch `XMLUI_E2E_DEV_PORT=5465 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/Tabs/Tabs.spec.ts xmlui/src/components/TimeInput/TimeInput.spec.ts xmlui/src/components/Toast/Toast.spec.ts --workers=1` passed 244/244. Waiting for user approval before marking Complete. |
...
| TimeInput | `TimeInput.tsx`, `TimeInputReact.tsx`, `TimeInput.defaults.ts`, `TimeInput.module.scss`, `TimeInput.md`, `TimeInput.spec.ts`, `utils.ts` | Input helpers, Form context, label behavior, locale parsing | In review | Strict copy migrated from original. `node xmlui/scripts/verify-protected-component-copy.mjs TimeInput` passed (`TimeInput.tsx` entry-adapted; copied React/defaults/SCSS/docs/spec/helper identical). Runtime adapter appended below copied entry; obsolete `TimeInput.renderer.tsx` removed; registries import the provider-style entry. Added the original `get-user-locale@^3.0.0` dependency. Host fixes stayed outside protected copied files: Input theme aliases, Form/bindTo/API value bridging, event mapping, tooltip/behavior trigger forwarding, and filtering of wrapper-injected structural data attributes before they reach the copied native root. Verification passed: `npm --prefix xmlui run check:metadata`; final combined copied E2E batch `XMLUI_E2E_DEV_PORT=5465 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/Tabs/Tabs.spec.ts xmlui/src/components/TimeInput/TimeInput.spec.ts xmlui/src/components/Toast/Toast.spec.ts --workers=1` passed 244/244. Waiting for user approval before marking Complete. |
...
| Toast | `Toast.tsx`, `ToastReact.tsx`, `Toast.spec.ts` | app globals, notifications, react-hot-toast | In review | Strict copy migrated from original. `node xmlui/scripts/verify-protected-component-copy.mjs Toast` passed (`Toast.tsx` entry-adapted; copied React/spec identical). Runtime adapter appended below copied entry; obsolete `Toast.renderer.tsx` removed; registries import the provider-style entry. Host adapter renders the copied toast templates with `$param` scoped data and clears hot toasts on mount/unmount to prevent cross-test leakage. Verification passed: `npm --prefix xmlui run check:metadata`; final combined copied E2E batch `XMLUI_E2E_DEV_PORT=5465 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/Tabs/Tabs.spec.ts xmlui/src/components/TimeInput/TimeInput.spec.ts xmlui/src/components/Toast/Toast.spec.ts --workers=1` passed 244/244. Waiting for user approval before marking Complete. |
...
- Tabs, TimeInput, and Toast are `In review`. Strict source was recopied from
  `/Users/dotneteer/source/xmlui/xmlui/src/components`; runtime adapters now
  live below the copied entry files, obsolete sidecar renderers were removed,
  and original dependencies `@radix-ui/react-tabs@1.1.13` and
  `get-user-locale@^3.0.0` were restored. Protected-copy audits pass for all
  three, metadata verification passes, and the final combined copied E2E batch
  passed 244/244:
  `XMLUI_E2E_DEV_PORT=5465 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/Tabs/Tabs.spec.ts xmlui/src/components/TimeInput/TimeInput.spec.ts xmlui/src/components/Toast/Toast.spec.ts --workers=1`.
  These components are waiting for user approval before they can be marked
  `Complete`.
- ScrollViewer, SelectionStore, SkipLink, and StickyBox are `Complete` after
  user approval.
  Protected-copy audits pass for all four, metadata verification passes, and the
  focused combined E2E batch passed 60/60:
  `XMLUI_E2E_DEV_PORT=5445 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/SkipLink/SkipLink.spec.ts xmlui/src/components/SelectionStore/SelectionStore.foundation.spec.ts xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1`.
  Strict source was recopied from `/Users/dotneteer/source/xmlui/xmlui/src/components`;
  runtime adapters now live below the copied entry files, obsolete sidecar
  renderers were removed, and original dependencies `overlayscrollbars`,
  `overlayscrollbars-react`, and `react-sticky-el` were restored. Shared host
  shims added outside protected files cover `useIsTouchDevice`,
  `useRealBackground`, ScrollViewer root attrs in header/footer mode,
  SelectionStore local update state/API value bridging, and StickyBox test-id
  hosting for the original `react-sticky-el` implementation.
...
- Completed-component regression cleanup after the broad `test:e2e` failure list:
  ...
  sibling and proving no additional parent is inserted. A later completed-component
  regression pass after the ScrollViewer/SkipLink/StickyBox/SelectionStore work
  fixed CodeBlock's runtime `data-xmlui-part` bridge and Table's `syncWithVar`
  initial-selection echo, then applied allowed E2E constraint loosening only to
  completed-component tests: FileUploadDropZone's border foundation fixture no
  longer includes an unrelated unsupported upload handler, Theme NavPanel/Footer
  cases assert visibility plus themed background instead of exact shell geometry,
  the Tree `getExpandedNodes()` API scenario keeps its behavioral assertions but
  has a 60s timeout for the long imperative sequence, and the standalone theming
  layout tests tolerate small RGB rounding drift while preserving scoped-theme
  and mutation-update checks. Verification passed for the focused completed slice
  `XMLUI_E2E_DEV_PORT=5465 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/FileUploadDropZone/FileUploadDropZone.foundation.spec.ts xmlui/src/components/Spinner/Spinner.spec.ts xmlui/src/components/Theme/Theme.spec.ts xmlui/src/components/Table/Table.spec.ts xmlui/src/components/Tree/Tree-dynamic.spec.ts xmlui/tests/e2e/theming-layout-styling.spec.ts --grep "default border color matches original runtime|component respects delay prop|button behind fullScreen spinner can't be clicked|Themed NavPanel|Themed Footer|initial selectedIds in the variable pre-selects|syncWithVar takes precedence|getExpandedNodes\\(\\)|nested theme scopes affect only their subtree|expression-backed styles update after data mutation" --workers=1`
  passed 10/10, and `npm --prefix xmlui run check:metadata` passed.
