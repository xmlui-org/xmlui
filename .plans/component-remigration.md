...
| `AppState` | More difficult | 1 | none | runtime state | Approved complete | User confirmed AppState is complete on 2026-06-30. Migrated at the transferred non-visual renderer boundary against `/Users/dotneteer/source/xmlui/xmlui/src/components/AppState/AppStateReact.tsx`, `AppState.tsx`, and the old AppState/FlowLayout specs. The renderer preserves bucket state sharing, initial value merging, `value`, `update`, `appendToList`, `removeFromList`, `listIncludes`, `didUpdate` payloads with previous values, and persistent bucket state after conditional unmount. Follow-up compatibility restores old `ref="appState"` API exposure in addition to `id`, allows arbitrary non-rendered props such as `testId`, and expands compiler reference discovery so `ref` names bind in expressions without creating a DOM wrapper. Compiler method-call generation now accepts the AppState API methods in expression arrows as well as event handlers, covering the `Checkbox onDidChange="v => appState.update(...)"` parent-to-nested-component sharing case. Focused AppState suite passes 9/9; TypeScript and metadata checks pass. |
...
| `Bookmark` | More difficult | 1 | `Bookmark.module.scss` | runtime navigation/state | Approved complete | User confirmed Bookmark is complete on 2026-07-01. Source-preserving migration checked against `/Users/dotneteer/source/xmlui/xmlui/src/components/Bookmark/BookmarkReact.tsx`, `Bookmark.tsx`, `Bookmark.defaults.ts`, and `Bookmark.module.scss`. Renderer preserves the original span anchor DOM, child rendering, `id`/`data-anchor`, old metadata/defaults/deprecation text, `scroll-margin-top: var(--header-height)`, component API exposure, old `ref` alias exposure, and the original `scrollIntoView` algorithm that scrolls the nearest scrollable ancestor before falling back to the target with `block: "start"`. Follow-ups fixed a routed App/Pages feedback loop by registering the component API through the stable adapter callback, restored the original `vertical-full-header`/`scrollWholePage=false` content-scroll container so the window does not scroll, restored no-header helper variables so the side NavPanel is not pushed down by the default header height, and bridged Link clicks through the runtime router so `to="/#id"` anchors scroll the bounded App content area. The user-provided three-bookmark layout is covered with 800px sections, window-scroll assertions, NavPanel/content-row alignment, and green/blue link clicks. The rewrite keeps TOC discovery data attributes for the current TableOfContents scanner while preserving `omitFromToc`, `level`, and `title` semantics. Focused Bookmark suite passes 7/7; focused Link suite passes 55/55; focused Pages suite passes 6/6; TypeScript and metadata checks pass. |
...
| `NavGroup` | Complex | 1 | `NavGroup.module.scss` | App, Icon, NavLink, NavPanel | Approved complete | User confirmed NavGroup is complete on 2026-06-30. Implemented as part of the app-shell/navigation batch; original stylesheet restored, old NavLink-trigger class contract restored, icon props bridged, disabled/expanded state preserved, and nested link click behavior verified. Follow-up parity for condensed/horizontal layouts restores the original source behavior from `/Users/dotneteer/source/xmlui/xmlui/src/components/NavGroup/NavGroupReact.tsx` and `/Users/dotneteer/source/xmlui/xmlui/src/components/NavGroup/NavGroup.tsx`: vertical and narrow drawer groups remain inline expanders, while condensed/horizontal groups render through Radix `DropdownMenu`/`DropdownMenuSub` with portaled dropdown content, level-aware submenu triggers, and horizontal/vertical expand icons. NavGroup triggers now also receive the NavLink theme class/style like the original `ThemedNavLink`, restoring `gap-icon-NavLink` spacing between labels and chevrons. Closed dropdowns restore focus to the trigger and paint the same NavLink focus outline via the NavLink focus CSS variables. The portaled dropdown receives the NavGroup theme class/style so `minWidth-dropdown-NavGroup`, dropdown surface styling, and NavGroup padding variables apply outside the trigger DOM. Dropdown children follow the original `renderChild(...wrapChild...)` semantics: NavLinks render as vertical Radix menu items and nested NavGroups remain submenu triggers. Nested dropdown NavGroup icons now receive the original NavLink icon class from `NavGroup.tsx`, so authored `icon` color matches sibling NavLink icons; regression `nested dropdown navgroup icon uses navlink icon color` covers the `Send To`/`Team` case. The focused regression `condensed layout opens navgroup menus in portals` verifies the user's nested `Pages` example, asserts the top-level trigger uses `chevrondown` with the old label/icon gap, checks the restored focus outline after close, verifies menu items render outside the NavPanel DOM with old minimum width, and confirms the `Page 2-4` submenu opens to the right as a separate portal with `chevronright`. Focused NavGroup suite now passes 8 active tests with 25 compatibility-debt skips; TypeScript passes. |
