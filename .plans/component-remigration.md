...
| Audit required | Component exists in the rewrite but has not passed the new protected-file audit. | 19 |
...
| Complete | User approved after audit and verification. | 90 |
...
| ScrollViewer | `ScrollViewer.tsx`, `ScrollViewerReact.tsx`, `Scroller.tsx`, `ScrollViewer.defaults.ts`, `ScrollViewer.module.scss`, `ScrollViewer.md`, `ScrollViewer.spec.ts`; added `ScrollViewer.foundation.spec.ts` | layout, Bookmark, original overlay scrollbar dependencies | In review | Strict copy migrated from original. `node xmlui/scripts/verify-protected-component-copy.mjs ScrollViewer` passed (`ScrollViewer.tsx` entry-adapted; copied React/Scroller/SCSS/defaults/docs/spec identical). Runtime adapter appended below copied entry; obsolete sidecar renderer removed and registries import the provider-style entry. Added original `overlayscrollbars@2.13.0` and `overlayscrollbars-react@0.5.6` dependencies. Host fixes stayed outside copied source: shared `useIsTouchDevice` shim, root-attribute bridge for copied header/footer wrapper mode, and an empty-viewer min-height host fallback so copied empty-content coverage remains visible. Verification passed: `npm --prefix xmlui run check:metadata`; focused failure slice `XMLUI_E2E_DEV_PORT=5442 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts --grep "renders scrollable content and header/footer templates|handles empty content|Header and Footer Templates" --workers=1` passed 12/12; final focused batch `XMLUI_E2E_DEV_PORT=5445 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/SkipLink/SkipLink.spec.ts xmlui/src/components/SelectionStore/SelectionStore.foundation.spec.ts xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 60/60. Waiting for user approval before marking Complete. |
...
| SkipLink | `SkipLink.tsx`, `SkipLinkReact.tsx`, `SkipLink.defaults.ts`, `SkipLink.md`, `SkipLink.spec.ts` | accessibility | In review | Strict copy migrated from original, replacing the interim rewrite-only module SCSS/renderer implementation with copied `SkipLinkReact.tsx` and defaults. `node xmlui/scripts/verify-protected-component-copy.mjs SkipLink` passed (`SkipLink.tsx` entry-adapted; copied React/defaults/docs/spec identical). Runtime adapter appended below copied entry; obsolete sidecar renderer and non-original module SCSS removed; registries import the provider-style entry. Verification passed: `npm --prefix xmlui run check:metadata`; final focused batch `XMLUI_E2E_DEV_PORT=5445 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/SkipLink/SkipLink.spec.ts xmlui/src/components/SelectionStore/SelectionStore.foundation.spec.ts xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 60/60. Waiting for user approval before marking Complete. |
...
| StickyBox | `StickyBox.tsx`, `StickyBoxReact.tsx`, `StickyBox.defaults.ts`, `StickyBox.module.scss`, `StickyBox.md`; updated `StickySection.foundation.spec.ts` | scroll containers, original sticky dependency | In review | Strict copy migrated from original. `node xmlui/scripts/verify-protected-component-copy.mjs StickyBox` passed (`StickyBox.tsx` entry-adapted; copied React/SCSS/defaults/docs identical). Runtime adapter appended below copied entry; obsolete sidecar renderer removed and registries import the provider-style entry. Added original `react-sticky-el@^2.1.1` dependency and restored shared `useRealBackground` host shim outside copied source. The unprotected Sticky foundation test was realigned from the interim native-sticky shim assertion to the migrated original `react-sticky-el` behavior by asserting rendered StickyBox content and keeping the existing StickySection mutation coverage. Verification passed: `npm --prefix xmlui run check:metadata`; `XMLUI_E2E_DEV_PORT=5444 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 3/3; final focused batch `XMLUI_E2E_DEV_PORT=5445 XMLUI_REUSE_EXISTING_SERVER=0 npm --workspace xmlui run test:e2e -- xmlui/src/components/ScrollViewer/ScrollViewer.spec.ts xmlui/src/components/ScrollViewer/ScrollViewer.foundation.spec.ts xmlui/src/components/SkipLink/SkipLink.spec.ts xmlui/src/components/SelectionStore/SelectionStore.foundation.spec.ts xmlui/src/components/StickySection/StickySection.foundation.spec.ts --workers=1` passed 60/60. Waiting for user approval before marking Complete. |
...
- ScrollViewer, SkipLink, and StickyBox are `In review`; SelectionStore is
  `Complete` after user approval.
  ...
  hosting for the original `react-sticky-el` implementation. ScrollViewer,
  SkipLink, and StickyBox are waiting for user approval before they can be
  marked `Complete`.
