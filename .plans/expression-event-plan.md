...
small, XMLUI-specific semantic IR that can be checked, compiled, tested, and
reused by language-server features.
...
6. compile the IR into JavaScript function bodies for the runtime slice;
...
For the initial runtime slice, compile checked IR into JavaScript functions that
receive a small explicit execution context rather than relying on `eval` or
...
Generated code should be simple before it is clever. The important result is
that the runtime calls compiled functions and does not interpret XMLUI script
ASTs on every render or event.
...
9. Compiler pipeline integration — completed
   - Replace any remaining script-specific runtime interpretation in the first
     experiment with semantic analysis plus compiled functions.
