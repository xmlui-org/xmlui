...
Each visual component has properties that you can use to set a particular visual trait of that component. XMLUI names them _layout properties_; you can learn their usage in the [Layout Properties article](./layout-props) article. The [Layout Properties reference](/components/appearance/layout-props) gives you a summary of the available properties and their values.
...
Layout properties are easy to use; adding simple property values can quickly change the visuals of a particular component. However, this ease comes at a high price: the more individual layout props there are, the more tedious the application maintenance is if you need to change the appearance of the app.

For example, changing the dimensions of boxes in the code snippet above affects many values (all `width` and `height` props).
...
To remove the pain of setting all `width` and `height` occurrences in the previous example (when using layout properties), define a reusable component, `MyBox` with the wired dimension values; however, allow them to receive props to set the border and color values:
...
Now, with `MyBox`, the previous UI declaration is more straightforward:
...
Let's assume, we need to change the dimensions from 80x40 to 60x60 pixels. Altering the `width` and `height` props within `MyBox` has its effect on all boxes:
...
You can extract the values of layout props into theme variables. For example, the hard-wired dimensions of `MyBox` can be put into theme variables:
...
When you build and deploy the app, you can opt to expose the themes to app admins so that they can modify them. Changing the dimensions of `MyBox` is straightforward by setting the appropriate theme variables:
