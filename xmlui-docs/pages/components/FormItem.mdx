...
A `FormItem` component represents a single input element within a `Form`.
The value within the `FormItem` may be associated with a particular property within the encapsulating `Form` component's data.
...
This property indicates that the `FormItem` should automatically receive focus when the form is focused.
...
This boolean property is used to set the `FormItem` active (`true`) or inactive (`false`).
...
This property is used to provide an initial value for the input field in the `FormItem` if the value that it is [bound to](#bindto) is undefined.
...
This string property sets a text label for the input field.
...
This optional property indicates where the label should be positioned next to the input field.
...
This optional boolean prop indicates whether the input field is required (`true`) or not (`false`) to fill out in order to submit the form.
...
The `type` property is used to determine the specific input control the FormItem will wrap around.
Note that the control names start with a lowercase letter and map to input components found in XMLUI.
