...
This property is an optional [size value](./appearance/common-units#size-values) to determine the gap between the icon and the label components (if both are set). The current theme determines the default value of this prop.

```ueml copy
<HStack gap="1rem" >
  <Button icon="trash" label="default gap" />
  <Button icon="trash" label="No gap" gap="0" />
  <Button icon="trash" label="12px gap" gap="12px" />
  <Button icon="trash" label="2rem gap" gap="2rem" />
</HStack>
```
