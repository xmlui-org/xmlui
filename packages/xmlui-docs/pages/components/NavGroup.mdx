...
The `NavGroup` component is a container for cohesive navigation targets (`NavLink` components). They can form a submenu in the App's UI.
...
This property defines an optional icon to display with the `NavGroup` label, as the following example shows:
...
<Playground
    previewOnly={true}
    name="Example: NavGroup labels and icons"
    height={220}
    app={`
    <HStack padding="0.5rem" verticalAlignment="center">
      <NavGroup icon="email" label="Send To" >
...
The following sample demonstrates using these theme variables:
