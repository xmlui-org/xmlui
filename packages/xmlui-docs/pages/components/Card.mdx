...
There are also prestyled properties one can make use of, detailed in the [Props section](#props).
...
Corresponds with the [`url` prop in Avatar](./Avatar.mdx#url).

### `linkTo`

The `linkTo` wraps the title in a `Link` component that is clickable to navigate.
See the [`to` prop in Link](./Link.mdx#to).
