...
Though inline scripts often shorten the code, they are inconvenient when you work with more extended code. Code-behind files are excellent for keeping component code together; however, you must page between the markup and code-behind files. 

XMLUI offers a third way: you can use `<script>` tags to encapsulate code elements and still merge markup and code snippets in a single file.
