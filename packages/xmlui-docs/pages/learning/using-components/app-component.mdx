...
Instead of starting with a fixed layout, you can choose from a set of predefined ones. Moreover, you can change the app layout run time or through configuration. `App` provides several scroll strategies (according to the selected layout) and a responsive layout according to the available viewport width.
