...
In the [Binding Expressions](./binding-expressions)
document, you could learn about the expression syntax. Here, you will learn the statement syntax.
...
> **Note**: Here, `idUsed` is the identifier used as the event handler, and `arg1`,  `arg2`, and `argN` are the event arguments.
...
> **Note**: Be aware of using the `return` statement explicitly to retrieve a value from an event handler.