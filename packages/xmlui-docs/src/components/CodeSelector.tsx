...
export const CodeSelector = () => {
  ...
  return (
    <RadixSelect.Root value={options.content} onValueChange={(value) => dispatch(contentChanged(value))}>
      <RadixSelect.Trigger className={selectStyles.SelectTrigger} aria-label="component">
        <RadixSelect.Value>{options.content}</RadixSelect.Value>
        <RadixSelect.Icon className={selectStyles.SelectIcon}>
          <ChevronDownIcon />
        </RadixSelect.Icon>
      </RadixSelect.Trigger>
      <RadixSelect.Portal>
        <RadixSelect.Content className={selectStyles.SelectContent} side="bottom" align="start" position="popper">
          <RadixSelect.Viewport>
            ...
            {appDescription.components?.length > 0 && (
              <RadixSelect.Group>
                <RadixSelect.Label className={selectStyles.SelectLabel}>Components</RadixSelect.Label>
